 ...
 * lives in IndexedDB instead of the NyouOS localStorage settings object.
 */
const DeveloperCenterStore = {
    DB_NAME: 'NyouOS.developerCenter',
    DB_VERSION: 3,
    PACKAGE_VERSION: 2,
    PACKAGE_EXTENSION: '.fap',
    PACKAGE_MIME: 'application/vnd.Nyou.app-package',
    ...
    UNSUPPORTED_BROWSER_API_RULES: Object.freeze([
        Object.freeze({ regex: /\bfetch\s*\(/i, name: 'fetch' }),
        Object.freeze({ regex: /\bXMLHttpRequest\b/i, name: 'XMLHttpRequest' }),
        Object.freeze({ regex: /\bWebSocket\b/i, name: 'WebSocket' }),
        Object.freeze({ regex: /\bEventSource\b/i, name: 'EventSource' }),
        Object.freeze({ regex: /navigator\s*\.\s*sendBeacon\s*\(/i, name: 'navigator.sendBeacon' }),
        Object.freeze({ regex: /\b(?:Worker|SharedWorker|WebAssembly)\b/i, name: 'worker or WebAssembly' }),
        Object.freeze({ regex: /<\s*(?:img|iframe|script)\b[^>]*(?:src)\s*=\s*["']?\s*https?:\/\//i, name: 'remote embedded resource' }),
        Object.freeze({ regex: /<\s*link\b[^>]*href\s*=\s*["']?\s*https?:\/\//i, name: 'remote stylesheet' }),
        Object.freeze({ regex: /(?:url\s*\(\s*|@import\s+)["']?\s*https?:\/\//i, name: 'remote CSS URL' }),
        Object.freeze({ regex: /\bindexedDB\b/i, name: 'indexedDB' }),
        Object.freeze({ regex: /\blocalStorage\b|\bsessionStorage\b/i, name: 'direct browser storage' }),
        Object.freeze({ regex: /navigator\s*\.\s*(?:clipboard|geolocation|mediaDevices|usb|serial|bluetooth)/i, name: 'privileged navigator API' })
    ]),
    PERMISSION_DETAILS: Object.freeze({
        'storage.local': Object.freeze({
            nameZh: '读写浏览器本地存储', nameEn: 'Read and write browser local storage',
            descriptionZh: '允许此 App 在浏览器中持久保存和读取自己的本地数据。PWA 使用其网站来源的 localStorage，自建 App 使用隔离的 NyouOS.storage。',
            descriptionEn: 'Allow this App to persist and read its own browser-local data. PWAs use localStorage for their site origin; created Apps use isolated NyouOS.storage.'
        }),
        'system.theme.write': Object.freeze({
            nameZh: '修改系统主题', nameEn: 'Change system theme',
            descriptionZh: '允许此 App 切换 Nyou 的明暗模式和主题外观。',
            descriptionEn: "Allow this App to change Nyou's light, dark, and theme appearance."
        }),
    ...
    async put(storeName, value) {
        ...
        globalThis.NyouOSStorage?.invalidate?.();
        return value;
    },
    ...
    async nameExists(name, excludeProjectId = null) {
        ...
        const isExcludedProject = (projectId) => !!excludeProjectId && projectId === excludeProjectId;
        return projects.some((item) => !isExcludedProject(item.id) && String(item.name || '').trim().toLocaleLowerCase() === normalized) ||
            apps.some((item) => !isExcludedProject(item.projectId) && String(item.name || '').trim().toLocaleLowerCase() === normalized);
    },
    ...
    async accountIdentity() {
        ...
        const deviceName = String(settings.deviceName || 'NyouOS-PC').trim() || 'NyouOS-PC';
    ...
    async exportApp(app) {
        const icon = String(app.icon || 'Theme/Icon/App_icon/created_app.webp');
        ...
        const entries = {};
        const manifest = {
            format: 'Nyouapp', version: this.PACKAGE_VERSION, id: app.id, projectId: app.projectId,
            name: app.name, type: app.type, title: app.title || app.name,
            description,
            developer,
            developerHash,
            allowUnpack: app.allowUnpack === true,
            forceNyouUI: app.forceNyouUI === true,
    ...
    async importApp(file) {
        ...
        if (!filename.toLowerCase().endsWith(this.PACKAGE_EXTENSION)) {
            throw new Error('Only Nyou .fap packages can be opened');
        }
        // .fap is a versioned Nyou package whose payload remains ZIP. Give the
        ...
        if (manifest.format !== 'Nyouapp' || manifest.version !== this.PACKAGE_VERSION) throw new Error('Unsupported Nyou App package');
        ...
        const icon = manifest.iconFile && entries[manifest.iconFile]
            ...
            : (manifest.iconPath || 'Theme/Icon/App_icon/created_app.webp');
        return {
            ...
            developer,
            developerHash,
            allowUnpack: manifest.allowUnpack === true,
            ...
            forceNyouUI: manifest.forceNyouUI === true,
            permissions,
            network
        };
    },
    ...
    inspectAppSafety(app) {
        ...
        if (app.type === 'pwa') {
            ...
            if (url.protocol !== 'https:' || url.username || url.password || privateHost) {
                return fail('malicious', 'The PWA URL does not meet Nyou safety requirements');
            }
        ...
        const apiMatch = this.UNSUPPORTED_BROWSER_API_RULES.find((rule) => rule.regex.test(combined));
        ...
        if (missingPermission) return fail('api', `Undeclared Nyou API permission: ${missingPermission.permission}`);
        try {
