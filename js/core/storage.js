...
const Storage = {
    keys: {
        SETTINGS: 'NyouOS.settings',
        SESSION: 'NyouOS.session',
        FS: 'NyouOS.fs',
        DESKTOP_LAYOUT: 'NyouOS.desktopLayout',
        APP_USAGE: 'NyouOS.appUsage',
        NOTIFICATIONS: 'NyouOS.notifications'
    },

    fsInlinePayloadMarker: '__NyouOS_removed_inline_payload__',
    ...
    parseFS(data, defaultValue = null) {
        ...
        if (cleaned.changed) {
            ...
            try {
                ...
                console.warn('[Storage] Removed oversized inline file payloads from NyouOS.fs to keep startup responsive.');
            } catch (error) {
        ...
        try {
            ...
            if (key === this.keys.FS) {
                ...
                if (cleaned.changed) {
                    ...
                    value = cleanedValue;
                    console.warn('[Storage] Prevented oversized inline file payloads from being written to NyouOS.fs.');
                }
            }
    ...
    // 彻底删除当前 NyouOS 源下的所有用户数据，而不是只重置设置。
    async clearAllUserData() {
        ...
        await attempt('IndexedDB', async () => {
            ...
            const names = new Set(['NyouOSWallpaperCache', 'NyouOS.photos.cache.v1', 'NyouOSMediaLibrary']);
    ...
    initDefaults() {
        ...
        if (!this.get(this.keys.SETTINGS)) {
            this.set(this.keys.SETTINGS, {
                theme: 'light',
                wallpaperDesktop: 'Theme/Picture/Fluent-2.webp',
                wallpaperLock: 'Theme/Picture/Fluent-1.webp',
                enableBlur: true,
                ...
                userAvatar: 'Theme/Profile_img/UserAva.webp',
                language: 'zh',
                volume: 50,
                brightness: 100,
