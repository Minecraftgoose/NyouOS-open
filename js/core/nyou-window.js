/**
 * NyouWindow - 全新的 Nyou 窗口设计框架 (核心)
 * ===================================================================
 * 这是一个集中式的窗口内容框架。任何应用只要在 init() 中调用
 * `NyouWindow.mount()`，即可获得统一的「侧边栏 + 白色圆角卡片」布局：
 *
 *   ┌───────────────────────────────────────────┐  窗口 (高斯模糊半透明卡片)
 ...
 *
 * 关键优势：所有视觉与动效都集中在本文件 + css/nyou-window.css 中，
 * 只需修改这两个核心文件即可让所有使用该框架的 App 全局同时生效。
 *
 * ------------------------------------------------------------------
 * 用法示例:
 *   const frame = NyouWindow.mount({
...
const NyouWindow = {
    ...
    initGlobalScrollbars() {
        ...
        const getBounceTargets = (scrollable) => {
            ...
            if (scrollable.classList.contains('fw-card')) {
                ...
                if (page?.querySelector(':scope > [data-fw-scroll-bounce-fixed]')) {
                    return Array.from(page.children)
                        .filter((el) => el instanceof HTMLElement)
                        .filter((el) => !el.hasAttribute('data-fw-scroll-bounce-fixed'));
                }
        ...
        const markScrollableActive = (el) => {
            ...
            el.classList.add('fw-scrollbar-active', 'Nyou-scrollbar-active');

            if (state.hoverLockTimer) clearTimeout(state.hoverLockTimer);
            refreshBounceTargets(state);
            const lockTargets = normalizeTargets([el, ...state.bounceTargets], el);
            lockTargets.forEach((target) => target.classList.add('Nyou-scroll-hover-locked'));
            state.hoverLockTimer = setTimeout(() => {
                lockTargets.forEach((target) => target.classList.remove('Nyou-scroll-hover-locked'));
        ...
        this._removeGlobalScrollbar = (el) => {
            ...
            stopBounce(state);
            state.rail.remove();
            el.classList.remove('fw-scroll-host', 'fw-scrollbar-active', 'Nyou-scrollbar-active', 'Nyou-scroll-hover-locked');
        ...
        document.addEventListener('wheel', (e) => {
            ...
            if (selfScrollTarget instanceof HTMLElement) {
                ...
                // propagation here prevents NyouWindow/page bounce handlers from
     ...
     * @param {Function} [options.getNavActiveId] - (id, instance) => string 将二级路由映射到父导航项
    ...
    mount(options = {}) {
        ...
        if (!container) {
            console.error('[NyouWindow] mount: container is required');
            return null;
        }
        ...
        if (sidebarSearchConfigured) {
            ...
            sidebarSearchIcon.src = NyouWindow._iconPath('Search', 'stroke');
        ...
        const getNavActiveId = () => {
            if (typeof opts.getNavActiveId !== 'function') return instance.activeId;
            const resolvedId = opts.getNavActiveId(instance.activeId, instance);
            return resolvedId == null ? instance.activeId : resolvedId;
        };
        ...
        const renderSidebarSearchResults = (results, renderOptions = {}) => {
            ...
            normalized.forEach(result => {
                ...
                if (result.iconSrc) {
                    ...
                    iconWrap.style.setProperty('--fw-icon-url', `url("${NyouWindow._iconPath(result.icon, 'stroke')}")`);
                } else {
                ...
                if (result.actions.length) {
                    ...
                    result.actions.forEach(action => {
                        ...
                        if (action.iconSrc) {
                            ...
                            actionIcon.style.setProperty('--fw-icon-url', `url("${NyouWindow._iconPath(action.icon, 'stroke')}")`);
        ...
        const runSidebarSearch = async (query) => {
            ...
            try {
                ...
                if (seq !== instance._sidebarSearchSeq) return;
                console.error('[NyouWindow] sidebarSearch error:', err);
        ...
        const renderNavItems = () => {
            ...
            const buildItem = (item, target) => {
                ...
                const isActive = item.id === getNavActiveId();
                ...
                if (item.icon) {
                    ...
                    const iconPath = NyouWindow._iconPath(item.icon, type);
        ...
        const renderPage = () => {
            ...
            try {
                ...
                console.error('[NyouWindow] onNavigate error:', err);
            }
        };
    ...
    _iconPath(name, type = 'stroke') {
        if (!name) return '';
        // 复用 NyouUI 的图标路径规则，保持系统一致
        let path = '';
        if (typeof NyouUI !== 'undefined' && NyouUI._utils && NyouUI._utils.getIconPath) {
            path = NyouUI._utils.getIconPath(name, type);
        } else {
        ...
        // css/nyou-window.css and repeatedly requests /css/Theme/....
        try {
...
if (typeof window !== 'undefined') {
    window.NyouWindow = NyouWindow;
    const initScrollbars = () => NyouWindow.initGlobalScrollbars();
