 ...
 */
const FavoriteSites = {
    STORAGE_KEY: 'NyouOS.favoriteSites',
    META_CACHE_KEY: 'NyouOS.favoriteSiteMeta',
    ASK_PREF_KEY: 'NyouOS.favoriteSiteAskPrefs',
    MIGRATION_KEY: 'NyouOS.favoriteSites.recommendationsMigrated.v1',
    ...
    saveSites(sites) {
        ...
        window.dispatchEvent(new CustomEvent('Nyou-favorite-sites-change', { detail: { sites: clean } }));
        return clean;
    },
    ...
    getCatalogApps() {
        ...
        return Array.isArray(window.NyouPWACatalog) ? window.NyouPWACatalog : [];
    },
    ...
    iconForApp(app) {
        ...
        return app.icon && app.icon.includes('/') ? app.icon : `Theme/Icon/App_icon/${app.icon || 'app_gallery.webp'}`;
    },
    ...
    notifyImport(title, message, type = 'info') {
        ...
        } else if (window.NyouUI?.Toast) NyouUI.Toast({ title, message, type });
    },
    ...
    showAppAvailableDialog(app, url, originKey) {
        if (typeof NyouUI === 'undefined' || !NyouUI.Dialog) {
            this.openInternal(url);
            return;
        }
