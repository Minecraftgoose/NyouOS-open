/**
 * User-controlled file upload into the in-memory FS (persisted via Storage).
...
(function () {
    ...
    function ensureStyles() {
        ...
        style.textContent = `
            body .file-import-modal .Nyou-modal {
                overflow: hidden;
            ...
            body.dark-mode .file-import-modal .Nyou-modal {
                background: rgba(12, 12, 12, 0.76);
                border-color: rgba(255, 255, 255, 0.14);
                box-shadow: 0 20px 60px rgba(0, 0, 0, 0.52);
            }
            .file-import-modal .Nyou-modal-content { padding: 14px 16px 16px; }
    ...
    function createSession(initialFolderId) {
        ...
        hint.textContent = isZh() ? '导入内容会保存在浏览器本地存储中' : 'Imported data is stored in browser local storage';

        const cancelBtn = NyouUI.Button
            ? NyouUI.Button({ text: t('files.import-cancel'), variant: 'secondary', onClick: () => session.cancel() })
            : null;
        const closeBtn = NyouUI.Button
            ? NyouUI.Button({ text: t('files.import-close'), variant: 'primary', onClick: () => session.close() })
            : null;
        ...
        const session = {
            ...
            _addTask(file, destFolderId) {
                ...
                const progress = NyouUI.Progress({ value: 0, max: 100, showLabel: false, variant: 'default' });
            ...
            async _run() {
                ...
                for (const task of this.tasks) {
                    ...
                    if (!isAllowedImportFile(task.file)) {
                        ...
                        task.progressEl.setValue(0);
                        task.bytesLoaded = 0;
                        NyouUI.Toast?.({ title: t('files.title'), message: unsupportedTypeMessage(task.file?.name), type: 'warning' });
                        this._refreshOverall();
                        continue;
                    }

                    // Product storage ceiling: capacity grows from 1 GB to
                    // 5 GB and then 10 GB, but imports may never exceed 10 GB.
                    const fileBytes = Math.max(0, Number(task.file.size || 0));
                    ...
                    if (importedBytesAtStart + reservedImportedBytes + fileBytes > maxImportBytes) {
                        ...
                        if (!capacityWarningShown) {
                            ...
                            NyouUI.Toast?.({
                                title: t('files.title'),
                                message: t('files.import-capacity-warning'),
                                type: 'warning'
                            });
                        }
                        this._refreshOverall();
                    ...
                    try {
                        ...
                        const node = {
                            ...
                            size: Number(task.file.size || 0),
                            mime: String(isPackageImportFile(task.file) ? 'application/vnd.Nyou.app-package' : (task.file.type || (cachedImage ? imageMimeForFile(task.file) : (cachedAudio ? 'audio/mpeg' : (isText ? 'text/plain' : 'application/octet-stream'))))),
                ...
                if (importedCount > 0) {
                    NyouUI.Toast?.({ title: t('files.title'), message: t('files.import-success', { count: importedCount }), type: 'success' });
                }
                if (hasError) {
                    NyouUI.Toast?.({ title: t('files.title'), message: t('files.import-failed'), type: 'warning' });
                }
    ...
    function importToFolder(folderId, fileList) {
        ...
        if (!window.NyouUI || typeof State === 'undefined') return;
