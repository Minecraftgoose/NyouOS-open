...
const Widgets = {
    ...
    _surfaceTransitionId: 0,
    _surfaceTransitionPromise: null,
    drawerPage: 'home',
    ...
    init() {
        ...
        window.addEventListener('Nyou-favorite-sites-change', () => {
    ...
    refreshWidgetsByPrefix(prefix) {
        ['desktop', 'lock'].forEach(surface => {
            ...
            this.getLayout()[surface].forEach(inst => {
                ...
                const el = layer.querySelector(`.Nyou-widget[data-instance-id="${inst.id}"]`);
                if (!def || !el) return;
                const body = el.querySelector('.Nyou-widget-body');
    ...
    async _refreshStaticBlurTexture() {
        ...
        const [desktopWallpaper, lockWallpaper] = typeof State.resolveWallpaper === 'function'
            ...
            : [
                (State.settings && State.settings.wallpaperDesktop) || 'Theme/Picture/Fluent-2.webp',
                (State.settings && State.settings.wallpaperLock) || 'Theme/Picture/Fluent-1.webp'
            ];
    ...
    _refreshStaticBlurTextureFor(surface, src) {
        ...
        img.onload = () => {
            ...
            try {
                ...
                // 与 .Nyou-widget-body 的 blur(var(--blur-lg)) saturate(150%) 保持观感一致
    ...
    _setStaticBlurSource(surface, src) {
        ...
        let safeSrc = String(src).replace(/\\/g, '/').replace(/"/g, '\\"');
        // Convert relative paths to absolute URLs so CSS variables resolve
        // correctly when used from stylesheets under css/ directory.
        if (safeSrc && !/^(https?:|data:|blob:|file:|\/)/i.test(safeSrc)) {
            try {
                safeSrc = new URL(safeSrc, location.href).href;
    ...
    _buildPreview(variant, scale) {
        ...
        inner.className = 'Nyou-widget widget-preview-inner';
        ...
        body.className = `Nyou-widget-body ${variant.theme || ''}`;
    ...
    _getWidgetGlowHit(source) {
        ...
        if (source.closest?.('.Nyou-widget-remove, .Nyou-widget-edit')) return null;

        const preview = source.closest ? source.closest('.widgets-preview-wrap') : null;
        if (preview) {
            const target = preview.querySelector('.Nyou-widget-body');
            return target ? { host: preview, target } : null;
        }

        const widget = source.closest ? source.closest('.Nyou-widget') : null;
        ...
        const target = widget.querySelector('.Nyou-widget-body');
    ...
    _ensureDefaultDesktopWidgets() {
        ...
        if (!layout.desktop.length) {
            ...
            const specs = [
                ...
                { id: 'mood', widgetId: 'mood-city-s', col: 2, row: 4 }
            ];
            specs.forEach(spec => {
    ...
    renderSurface(surface) {
        const layer = this.layers[surface];
        if (!layer) return;
        layer.querySelectorAll('.Nyou-widget').forEach(el => el.remove());
        const m = this._metrics(surface);
        this.getLayout()[surface].forEach(inst => {
            const def = this.registry.find(d => d.id === inst.widgetId);
            if (!def) return;
    ...
    _makeWidgetEl(inst, def, m, surface) {
        ...
        const body = document.createElement('div');
        body.className = `Nyou-widget-body ${def.theme || ''}`;
        el.appendChild(body);

        const removeBtn = document.createElement('button');
        removeBtn.className = 'Nyou-widget-remove button-glow-disabled';
        ...
        if (typeof def.renderEditor === 'function') {
            ...
            editBtn.className = 'Nyou-widget-edit button-glow-disabled';
        ...
        el.addEventListener('pointerdown', (e) => {
            ...
            if (e.target.closest('.Nyou-widget-remove, .Nyou-widget-edit')) return;
    ...
    openWidgetEditor(surface, instanceId) {
        ...
        const sourceEl = this.layers[surface]?.querySelector(`.Nyou-widget[data-instance-id="${instanceId}"]`);
        if (!sourceEl) return;
        ...
        frontBody.className = `Nyou-widget-body ${def.theme || ''}`;
    ...
    /** 「完成」：退出编辑、收起抽屉、恢复之前最小化的窗口 */
        ...
        const lockExit = this.lockEditMode
            ? this._exitLockEdit()
            : (this._surfaceTransitionPromise || Promise.resolve(true));
        ...
        Promise.resolve(lockExit).finally(() => this._restoreMinimizedWindows());
    },
    ...
    async _enterLockEdit() {
        if (this.lockEditMode) return false;
        const transitionId = ++this._surfaceTransitionId;
        this.lockEditMode = true;
        this._surfaceTransitioning = true;
        this.updateTexts();
        try {
            // 保持 State.view 为 desktop，因此返回桌面仍无需输入密码；
            // 视觉层则与标准锁屏键复用完全相同的景深转场。
            if (typeof window.runDesktopLockDepthTransition === 'function') {
                const transition = window.runDesktopLockDepthTransition('lock');
                this._surfaceTransitionPromise = transition;
                this.renderSurface('lock');
                await transition;
            } else {
                LockScreen.show();
            ...
            if (this._surfaceTransitionId === transitionId) {
                this._surfaceTransitioning = false;
                this._surfaceTransitionPromise = null;
            }
        }
    },
    ...
    async _exitLockEdit() {
        ...
        try {
            if (typeof window.runDesktopLockDepthTransition === 'function') {
                const transition = window.runDesktopLockDepthTransition('desktop');
                this._surfaceTransitionPromise = transition;
                this.renderSurface('desktop');
                await transition;
            } else {
    ...
    _startDrag(e, pending) {
        ...
        ghostBody.className = `Nyou-widget-body ${def.theme || ''}`;
    ...
    shouldSuppressLockUnlock(event) {
        if (this.lockEditMode || this._surfaceTransitioning) return true;
        if (event?.target?.closest?.('.Nyou-widget')) return true;
