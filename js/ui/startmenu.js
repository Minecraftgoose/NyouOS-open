...
const StartMenu = {
    ...
    renderApps() {
        ...
        pinnedApps.forEach(app => {
            ...
            const repairing = typeof SettingsApp !== 'undefined' && typeof SettingsApp.isAppRepairing === 'function' && SettingsApp.isAppRepairing(app.id);
    ...
    openAppAndClose(appId, data = null) {
        ...
        WindowManager.openApp(appId, data, { source: 'start-menu' });
        this.close();
        return true;
    },
    bindEvents() {
    ...
    openFolder(folderId) {
        ...
        WindowManager.openApp('files', null, { source: 'start-menu' });

        setTimeout(() => {
            const node = typeof State.findNode === 'function' ? State.findNode(targetId) : null;
            if (node?.type === 'folder' &&
                WindowManager.getAppComponent?.('files')?.navigateToId) {
                WindowManager.getAppComponent('files').navigateToId(targetId);
                return;
            }
    ...
    openRecentFile(fileId, filePath = '') {
        ...
        if (node?.type === 'file') {
            if (typeof FilesApp !== 'undefined' && typeof FilesApp.openNodeWithDefaultApp === 'function') {
                const opened = FilesApp.openNodeWithDefaultApp(node, { source: 'start-menu' });
                if (opened) return;
            }
            WindowManager.openApp('notes', { fileId: id }, { source: 'start-menu' });
            return;
        }
            ...
            switch (action) {
                ...
                case 'reveal-in-folder':
                    ...
                    setTimeout(() => {
                        ...
                        if (filesComponent?.navigateToId) {
                            // 如果是文件，先定位到父目录
                            let targetId = id;
                            ...
                            filesComponent.navigateToId(targetId, id); // 第二个参数用于高亮选中
                        }
                    }, 120);
                    this.close();
                    break;
    ...
    /**
     * 鼠标移动超过阈值即可拖拽；触控必须静止长按 1 秒后才进入拖拽模式。
     * 长按前的手指移动完全交给列表的原生滚动，避免滑动时误拖 App。
     */
    _onAppPointerDown(e) {
        ...
        // 触控不能在按下阶段阻止默认行为，否则浏览器无法启动原生惯性滚动。
        ...
        const beginDragging = (ev) => {
            ...
            appEl.classList.remove('touch-drag-armed');
            this.close();
        ...
        const onMove = (ev) => {
            ...
            if (!dragging) {
                if (distance < (isTouch ? 4 : 10)) return;
                beginDragging(ev);
            }
            ghost.style.left = `${ev.clientX}px`;
            ghost.style.top = `${ev.clientY}px`;
        ...
        const onUp = (ev) => {
            ...
            appEl.classList.remove('touch-drag-armed');
            try {
                if (appEl.hasPointerCapture?.(pointerId)) appEl.releasePointerCapture(pointerId);
            } catch (_) {}
            if (!dragging) {
                if (isTouch && dragArmed) setTimeout(() => { this._appDragging = false; }, 0);
                return;
            }
        ...
        if (isTouch) {
            holdTimer = setTimeout(() => {
                holdTimer = 0;
                if (!listenersAttached || !appEl.isConnected) return;
                dragArmed = true;
                this._appDragging = true;
                appEl.classList.add('touch-drag-armed');
                try { appEl.setPointerCapture?.(pointerId); } catch (_) {}
    ...
    showAppContextMenu(event, appId) {
        ...
        const isRepairing = typeof SettingsApp !== 'undefined' && typeof SettingsApp.isAppRepairing === 'function' && SettingsApp.isAppRepairing(appId);
