...
function wOpenInBrowser(input, ctx) {
    ...
    setTimeout(() => {
        const browserComponent = WindowManager?.getAppComponent?.('browser');
        if (browserComponent?.navigate) {
            browserComponent.navigate(input);
        }
    }, 450);
}
...
const W_PHOTO_FALLBACK_IMAGE = 'Theme/Picture/Fluent-2.webp';
const W_BING_CACHE_KEY = 'NyouOS.photos.bingCache.v1';
...
function wBingFallbackImage() {
    ...
    return wNormalizeBingImage({
        ...
        copyright: wUiText('NyouOS \u58c1\u7eb8', 'NyouOS wallpaper'),
        title: wUiText('NyouOS \u58c1\u7eb8', 'NyouOS wallpaper'),
...
function wMediaSnapshot() {
    ...
    let manifest = [];
    try {
        manifest = JSON.parse(localStorage.getItem('NyouOS.media.library.v1') || '[]');
    } catch (_) { manifest = []; }
...
function wQuickNotesText() {
    const notesComponent = WindowManager?.getAppComponent?.('notes') || window.NotesApp;
    if (notesComponent && typeof notesComponent.getQuickNoteContent === 'function') {
        return notesComponent.getQuickNoteContent();
    }
...
function wSetQuickNotesText(value) {
    ...
    const notesComponent = WindowManager?.getAppComponent?.('notes') || window.NotesApp;
    if (notesComponent && typeof notesComponent.setQuickNoteContent === 'function') {
        notesComponent.setQuickNoteContent(text, { source: 'widget' });
        return;
    }
    ...
    apps: [
        {
            ...
            nameKey: '位置心情',
            descKey: '显示当前城市和心情',
            icon: 'Theme/Icon/Symbol_icon/stroke/Home.svg',
            variants: [
                {
                    id: 'mood-city-s',
                    w: 2, h: 2,
                    sizeKey: '小',
                    theme: 'w-mood-city',
                    defaultSettings: { city: '深圳' },
                    render(body, ctx) {
                        const city = (ctx.instance?.settings?.city) || State?.settings?.userCity || '深圳';
                        body.innerHTML = `
                            <div class="mood-city-widget">
                                <div class="mood-city-emoji">😊</div>
                                <div class="mood-city-text">我在</div>
                                <div class="mood-city-name">${city}</div>
                                <div class="mood-city-slogan">天天开心哦!</div>
                            </div>
                        `;
                    },
                    renderEditor(body, ctx) {
                        const city = (ctx.instance?.settings?.city) || State?.settings?.userCity || '深圳';
                        body.innerHTML = `
                            <div style="padding:12px">
                                <div style="font-size:13px;font-weight:600;margin-bottom:8px">城市名称</div>
                                <input type="text" class="mood-city-input" value="${city}" placeholder="输入城市" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:6px;box-sizing:border-box">
                            </div>
                        `;
                        const input = body.querySelector('.mood-city-input');
                        if (input) {
                            input.addEventListener('change', () => {
                                ctx.setSettings({ city: input.value.trim() || '深圳' });
                                if (State?.updateSettings) State.updateSettings({ userCity: input.value.trim() || '深圳' });
                            });
                        }
                    },
                    onClick(ctx) {
