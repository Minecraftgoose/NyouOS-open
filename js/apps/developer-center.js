...
const DeveloperCenterApp = {
    ...
    _professionalOnboardingProjectId: null,
    _professionalApiReminderProjectId: null,
    _professionalOnboardingDialogShown: false,
    _lastAiSkillNotificationAt: 0,
    ...
    TEXT: {
        zh: {
            ...
            buildTitle: '构建 App', buildDesc: '从网站或代码创建属于你的 NyouOS App。', pwa: '构建 PWA App',
            ...
            forceNyou: '强制使用 NyouUI 组件库', forceNyouDesc: '用 NyouOS 内置样式接管按钮、输入框、选择框、勾选框、滑块和滚动条。',
            saveDraft: '暂存项目', package: '一键封装', apiDocs: 'API 参考文档', saved: '项目已暂存',
            downloadAiSkill: '下载 AI Skill',
            apiDocsReminder: '开始构建前建议先查看 API 参考文档，确认 Nyou 可以封装的能力。',
            apiDocsAiSkillNotice: '尝试使用 AI Skill，快速构建完美且符合 Nyou API 标准的 App。',
        ...
        en: {
            ...
            buildTitle: 'Build App', buildDesc: 'Create your own NyouOS App from a website or code.', pwa: 'Build PWA App',
            ...
            forceNyou: 'Force NyouUI components', forceNyouDesc: 'Use NyouOS controls for buttons, inputs, selects, checks, sliders, and scrollbars.',
            saveDraft: 'Save draft', package: 'Package now', apiDocs: 'API reference', saved: 'Project saved',
            downloadAiSkill: 'Download AI Skill',
            apiDocsReminder: 'Review the API reference before building to confirm which capabilities Nyou can package.',
            apiDocsAiSkillNotice: 'Try the AI Skill to quickly build a polished App that follows the Nyou API standard.',
    ...
    PERMISSIONS: [
        ...
        { id: 'system.theme.write', titleZh: '修改系统主题', titleEn: 'Change system theme', descZh: '允许切换浅色、深色或自动主题。', descEn: 'Switch NyouOS between light, dark, and automatic themes.' },
        ...
        { id: 'network.request', titleZh: '请求网络 API', titleEn: 'Request network APIs', descZh: '仅允许经 NyouOS.network.request 访问 App 声明的 API 域名。', descEn: 'Access only declared API domains through NyouOS.network.request.' },
        { id: 'network.image', titleZh: '加载网络图片', titleEn: 'Load network images', descZh: '仅允许经 NyouOS.network.loadImage 加载 App 声明域名上的图片。', descEn: 'Load images only from declared domains through NyouOS.network.loadImage.' }
    ],
            ...
            onNavigate: (id, pageEl) => {
                ...
                if (id === 'build' && this.activeProjectId) {
                    ...
                    DeveloperCenterStore.get('projects', projectId).then((project) => {
                        if (!this.canViewProjectSource(project) || this.activeProjectId !== projectId) return;
    ...
    button(text, variant, onClick, icon = null) {
        return NyouUI.Button({ text, variant: variant || 'secondary', icon, onClick });
    },

    toast(message, type = 'success') {
        NyouUI.Toast({ title: this.text('title'), message, type });
    },
    ...
    canViewProjectSource(project) {
        return !!project && (project.importedFromFap !== true || project.allowUnpack === true);
    },

    isDeveloperCenterApp(app) {
        return !!app && app.developerCenterVisible !== false;
    },
    ...
    async renderHome(page) {
        ...
        const [allProjects, favorites] = await Promise.all([DeveloperCenterStore.getAll('projects'), DeveloperCenterStore.getFavorites()]);
        const projects = allProjects.filter((project) => this.canViewProjectSource(project));
    ...
    createProjectCard(project) {
        ...
        card.tabIndex = 0;
        card.innerHTML = `<div class="dc-project-top"><img src="${this.esc(project.icon || 'Theme/Icon/App_icon/created_app.webp')}" alt=""><strong>${this.esc(project.name)}</strong></div><div class="dc-project-meta"><span class="dc-badge">${project.type === 'pwa' ? 'PWA' : 'HTML / JS'}</span><span>${this.formatDate(project.modifiedAt)}</span></div>`;
    ...
    async renderBuild(page) {
        ...
        const projects = (await DeveloperCenterStore.getAll('projects')).filter((project) => this.canViewProjectSource(project));
        ...
        NyouUI.InputDialog({
            ...
            onConfirm: async (value) => {
                ...
                if (type === 'professional') {
                    const onboarding = await DeveloperCenterStore.get('preferences', 'professional-app-api-onboarding-v1');
                    this._professionalOnboardingProjectId = onboarding?.value === true ? null : '__pending__';
                }
                const project = await DeveloperCenterStore.saveProject({
                    name, type, title: name, url: type === 'pwa' ? 'https://' : '',
                    icon: 'Theme/Icon/App_icon/created_app.webp',
                    html: type === 'professional' ? '<main>\n  <h1>Hello, NyouOS!</h1>\n  <button id="hello">Try NyouOS API</button>\n</main>' : '',
                    css: type === 'professional' ? 'main { padding: 32px; }' : '',
                    js: type === 'professional' ? "document.querySelector('#hello')?.addEventListener('click', () => {\n  NyouOS.notify('Hello', 'Your App is running!', 'success');\n});" : '',
                    forceNyouUI: type === 'professional',
                    permissions: [],
                    network: { connect: [], image: [] }
                });
                if (this._professionalOnboardingProjectId === '__pending__') this._professionalOnboardingProjectId = project.id;
    ...
    async openProject(projectId) {
        ...
        if (!this.canViewProjectSource(project) || !this.frame?.pageEl) return;
        if (project.type === 'professional' && this._professionalOnboardingProjectId !== project.id) {
            const onboarding = await DeveloperCenterStore.get('preferences', 'professional-app-api-onboarding-v1');
            this._professionalApiReminderProjectId = onboarding?.value === true ? project.id : null;
        }
        this._afterSecondaryExit(() => {
    ...
    renderPwaEditor(page, project) {
        ...
        form.innerHTML = `
            ...
            <div class="dc-field"><span class="dc-field-label">${this.text('icon')}</span><div class="dc-icon-picker"><img class="dc-icon-preview" src="${this.esc(project.icon || 'Theme/Icon/App_icon/created_app.webp')}" alt=""><button type="button" class="nyou-btn nyou-btn-secondary nyou-btn-medium" data-action="upload-icon"><span class="nyou-btn-text">${this.text('upload')}</span></button><input class="dc-hidden-input" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml"></div></div>
    ...
    renderProfessionalEditor(page, project) {
        ...
        this.mountToggle(layout.querySelector('[data-toggle="force"]'), this.text('forceNyou'), this.text('forceNyouDesc'), project.forceNyouUI === true, (value) => { project.forceNyouUI = value; });
        this.renderPermissionPicker(layout.querySelector('[data-permission-picker]'), project.permissions || []);
        const apiDocsAction = document.createElement('div');
        apiDocsAction.className = 'dc-api-action';
        apiDocsAction.appendChild(this.button(this.text('apiDocs'), 'secondary', () => this.showApiDocs(), 'Book Text'));
        if (this._professionalOnboardingProjectId === project.id) {
            queueMicrotask(() => this.showProfessionalOnboarding(project));
        }
        if (this._professionalApiReminderProjectId === project.id) {
            this._professionalApiReminderProjectId = null;
            queueMicrotask(() => this.toast(this.text('apiDocsReminder'), 'info'));
        }
        this.appendFooter(page, [
            apiDocsAction,
    ...
    mountToggle(host, title, description, checked, onChange) {
        ...
        host.appendChild(NyouUI.Toggle({ checked, onChange }));
    },
    ...
    renderPermissionPicker(host, selectedPermissions, allowedPermissions = DeveloperCenterStore.SUPPORTED_PERMISSIONS) {
        ...
        const english = typeof I18n !== 'undefined' && I18n.currentLang === 'en';
        host.innerHTML = `<strong class="dc-permission-heading">${english ? 'Optional permissions' : '可选权限'}</strong><span class="dc-permission-help">${english ? 'Select only capabilities this App needs. Local storage is authenticated before the App first opens; other capabilities are authenticated on first use.' : '只选择 App 实际需要的能力；本地存储会在 App 首次打开前认证，其他能力在首次调用时认证。'}</span>`;
    ...
    collectEditor(page, project) {
        const next = { ...project, icon: project.icon || 'Theme/Icon/App_icon/created_app.webp' };
        ...
        next.forceNyouUI = project.forceNyouUI === true;
    ...
    async renderPackageInfo(page, project) {
        ...
        form.innerHTML = `
            ...
            <label class="dc-package-unpack-row"><input type="checkbox" data-package-field="allow-unpack" ${project.allowUnpack === true ? 'checked' : ''}><span><strong>${english ? 'Allow other users to unpack this App' : '允许其他用户解包此 App'}</strong><small>${english ? 'When imported through Developer Center, the project source can be viewed and edited. Imports opened elsewhere never expose source in Developer Center.' : '勾选后，其他用户从开发者中心导入时可查看和编辑项目源码；从其他位置打开安装包时始终不会在开发者中心公开源码。'}</small></span></label>`;
        page.appendChild(form);
        this.appendFooter(page, [
    ...
    async savePackageInfo(page, project, identity) {
        ...
        const allowUnpack = !!page.querySelector('[data-package-field="allow-unpack"]:checked');
    ...
    apiDocsContent() {
        const content = `<div class="dc-api-docs">
            <div class="dc-api-row"><code>await NyouOS.notify(title, message, type)</code><span>Show an info, success, warning, or error notification.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.alert(title, message) / confirm(title, message)</code><span>Show a host-rendered system dialog. confirm() resolves to true or false.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.dialog({ title, message, type, buttons })</code><span>Show a custom dialog with up to three text-only buttons and resolve to the selected button value.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.getTheme()</code><span>Read mode, isDark, accentColor, accentRgb, NyouUI version mode, material, language, and window ID.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.getThemeMode() / getAccentColor() / getLanguage()</code><span>Read individual theme values. NyouOS.state contains the latest synchronously cached system state.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.isWindowBlurEnabled()</code><span>Read whether window blur is currently effective. The same value is available as NyouOS.state.windowBlurEnabled.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.storage.get(key) / set(key, value) / remove(key)</code><span>Permission: storage.local. Use private browser-local storage for this App: 100 KiB per JSON value, 128 keys, and 512 KiB total UTF-8 data.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.openApp(id) / openExternal(httpsUrl)</code><span>Open an allowed system App or a secure external link.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.clipboard.read() / write(text)</code><span>Permissions: clipboard.read / clipboard.write. Read or write text when browser permission also allows it.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>await NyouOS.getWindowInfo()</code><span>Read this App window's title, size, and maximized state.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row"><code>NyouOS.ui.highlightButton('#save', true) / enableButtonGlow('#save', true)</code><span>Apply the Nyou primary-button appearance or system pointer highlight feedback to matching controls.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.system.setTheme('dark') / toggleTheme()</code><span>Permission: system.theme.write. Change the global NyouOS theme.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.window.setTitle(title) / setSize(width, height)</code><span>Permission: window.manage. Manage only this App's window.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.files.listText('documents') / readText(id) / writeText(id, content) / createText(name, content)</code><span>Permissions: files.readText / files.writeText. List only Documents, Downloads, or Desktop; text content is limited to 512 KB and new files go to Documents.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.desktop.addShortcut() / removeShortcut()</code><span>Permission: desktop.manage. Manage only this App's own desktop shortcut.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>await NyouOS.network.request(url, options)</code><span>Permission: network.request. Request only an exact HTTPS hostname declared in manifest.network.connect. Returns status, headers, and a size-limited text body.</span><button type="button" data-copy-api>Copy</button></div>
            <div class="dc-api-row dc-api-privileged"><code>const src = await NyouOS.network.loadImage(url)</code><span>Permission: network.image. Loads a size-limited raster image from manifest.network.image; if CORS blocks fetch, it returns the allowlisted URL for CSP-restricted img.src without a host-side probe.</span><button type="button" data-copy-api>Copy</button></div></div>`;
        return content;
    },
    ...
    showApiDocs() {
        ...
        WindowManager.appConfigs['developer-center-api-docs'] = {
            ...
            icon: 'Theme/Icon/App_icon/developer_center.webp',
        ...
        this.notifyAiSkillSuggestion();
    },

    notifyAiSkillSuggestion() {
        const now = Date.now();
        if (now - this._lastAiSkillNotificationAt < 800) return;
        this._lastAiSkillNotificationAt = now;
        const title = this.text('apiDocs');
        const message = this.text('apiDocsAiSkillNotice');
        NyouUI.Toast({
            title,
            message,
            type: 'info',
    ...
    async showProfessionalOnboarding(project) {
        if (!project?.id || this._professionalOnboardingProjectId !== project.id || this._professionalOnboardingDialogShown) return;
        const onboarding = await DeveloperCenterStore.get('preferences', 'professional-app-api-onboarding-v1');
        if (onboarding?.value === true) {
            this._professionalOnboardingProjectId = null;
            return;
        }
        this._professionalOnboardingDialogShown = true;
        const english = typeof I18n !== 'undefined' && I18n.currentLang === 'en';
        NyouUI.Dialog({
            type: 'info',
            title: english ? 'Read the API reference before building' : '构建前请先阅读 API 参考文档',
            content: english
                ? 'Professional Apps run in a secure sandbox. Please read the API reference before writing code to avoid using capabilities that Nyou cannot package.'
                : '专业 App 会在安全沙箱中运行。请在编写代码前阅读 API 参考文档，避免使用 Nyou 无法封装的能力。',
            closeOnOverlay: false,
            buttons: [{ text: english ? 'OK' : '确定', variant: 'primary', value: true }],
            onClose: async () => {
                await DeveloperCenterStore.put('preferences', { key: 'professional-app-api-onboarding-v1', value: true });
                this.showApiDocs();
            }
        });
    },
    ...
    downloadAiSkill() {
        ...
        link.href = 'docs/Nyou-app-builder/SKILL.txt';
    ...
    async startValidation(page, project, options = {}) {
        ...
        const checks = project.type === 'pwa'
            ...
            : [
                ...
                { id: 'apis', label: 'Permitted APIs', desc: 'Only the provided NyouOS bridge is used' },
    ...
    async validatePwa(project, update, runId) {
        ...
        update('apis', apis ? 'pass' : 'fail', apis ? permissionSummary : 'PWA requests an unsupported permission or network allowlist', apis ? [] : [{ title: 'PWA permission rejected', message: 'PWAs may only request storage.local and cannot use NyouOS network allowlists.', file: 'App manifest' }]);
    ...
    async validateProfessional(project, update, runId) {
        ...
        update('apis', 'running');
        const apiRules = DeveloperCenterStore.UNSUPPORTED_BROWSER_API_RULES;
        ...
        const permissionRules = [
            { regex: /NyouOS\s*\.\s*storage\s*\.\s*(?:get|set|remove)\s*\(/, permission: 'storage.local' },
            { regex: /NyouOS\s*\.\s*call\s*\(\s*['"]storage\.(?:get|set|remove)['"]/, permission: 'storage.local' },
            { regex: /NyouOS\s*\.\s*system\s*\.\s*(?:setTheme|toggleTheme)\s*\(/, permission: 'system.theme.write' },
            { regex: /NyouOS\s*\.\s*window\s*\.\s*(?:setTitle|setSize)\s*\(/, permission: 'window.manage' },
            { regex: /NyouOS\s*\.\s*files\s*\.\s*(?:listText|readText)\s*\(/, permission: 'files.readText' },
            { regex: /NyouOS\s*\.\s*files\s*\.\s*(?:writeText|createText)\s*\(/, permission: 'files.writeText' },
            { regex: /NyouOS\s*\.\s*desktop\s*\.\s*(?:addShortcut|removeShortcut)\s*\(/, permission: 'desktop.manage' },
            { regex: /NyouOS\s*\.\s*clipboard\s*\.\s*read\s*\(/, permission: 'clipboard.read' },
            { regex: /NyouOS\s*\.\s*call\s*\(\s*['"]clipboard\.read['"]/, permission: 'clipboard.read' },
            { regex: /NyouOS\s*\.\s*clipboard\s*\.\s*write\s*\(/, permission: 'clipboard.write' },
            { regex: /NyouOS\s*\.\s*call\s*\(\s*['"]clipboard\.write['"]/, permission: 'clipboard.write' },
            { regex: /NyouOS\s*\.\s*network\s*\.\s*request\s*\(/, permission: 'network.request' },
            { regex: /NyouOS\s*\.\s*network\s*\.\s*loadImage\s*\(/, permission: 'network.image' }
        ];
        ...
        unsupportedMatches.forEach((match) => {
            const message = `Use the NyouOS API instead of ${match.name}`;
            apiProblems.push(message);
            apiDiagnostics.push(this.validationDiagnostic(
                match,
        ...
        update('apis', apiProblem ? 'fail' : 'pass', apiSummary || 'Only declared sandbox and NyouOS APIs are referenced', apiDiagnostics);
                ...
                frame = DeveloperCreatedRuntime.mountAppFrame(
                    ...
                    this.windowId,
                    {
                        ...
                        onRuntimeError: (message, metadata = {}) => {
                            if (!String(message).includes('This validation session permits read-only NyouOS APIs only')) {
    ...
    async finishPackaging(project, addDesktop) {
        ...
        const app = await DeveloperCenterStore.saveApp({
            ...
            allowUnpack: project.allowUnpack === true,
            developerCenterVisible: true,
        ...
        DeveloperCreatedRuntime.clearPermissionDecisions(app.id);
    ...
    async finishImportedApp(imported) {
        ...
        imported.developerCenterVisible = true;
        if (imported.allowUnpack === true) {
            const project = await DeveloperCenterStore.saveProject({
                ...
                css: imported.css,
                js: imported.js,
                forceNyouUI: imported.forceNyouUI === true,
            ...
            imported.projectId = imported.projectId || DeveloperCenterStore.createId('sealed-project');
        }
    ...
    async renderMyApps(page) {
        ...
        input.type = 'file';
        input.accept = '.fap,application/vnd.Nyou.app-package';
        input.className = 'dc-hidden-input dc-import-input';
        input.addEventListener('change', () => this.importPackage(input.files?.[0]));
        page.appendChild(input);
        const apps = (await DeveloperCenterStore.getAll('apps')).filter((app) => this.isDeveloperCenterApp(app));
    ...
    createAppCard(app) {
        ...
        card.innerHTML = `<div class="dc-project-top"><img src="${this.esc(app.icon || 'Theme/Icon/App_icon/created_app.webp')}" alt=""><strong>${this.esc(app.name)}</strong></div><div class="dc-project-meta"><span class="dc-badge">${app.type === 'pwa' ? 'PWA' : 'HTML / JS'}</span><span>${this.formatDate(app.modifiedAt)}</span></div><div class="dc-app-actions"></div>`;
    ...
    async exportPackage(app) {
        try {
            ...
            if (verified.sourcePackageId !== String(app.id || '') ||
                verified.name !== String(app.name || '').trim() ||
                verified.type !== app.type ||
                verified.allowUnpack !== (app.allowUnpack === true)) {
    ...
    async searchAll(query) {
        ...
        let [projects, apps] = await Promise.all([
            DeveloperCenterStore.getAll('projects'),
            DeveloperCenterStore.getAll('apps')
        ]);
        projects = projects.filter((project) => this.canViewProjectSource(project));
        apps = apps.filter((app) => this.isDeveloperCenterApp(app));
        ...
        const projectResults = projects.filter((project) => includes(`${project.name} ${project.type}`)).map((project) => ({
            ...
            iconSrc: project.icon || 'Theme/Icon/App_icon/created_app.webp',
        ...
        const appResults = apps.filter((app) => includes(`${app.name} ${app.type}`)).map((app) => ({
            ...
            iconSrc: app.icon || 'Theme/Icon/App_icon/created_app.webp',
    ...
    async renderSettings(page) {
        const renderToken = page.dataset.dcRenderToken;
        page.className = 'fw-page dc-page';
        let [projects, apps, bytes] = await Promise.all([
        ...
        editorSettings.innerHTML = `
            ...
            <pre class="dc-editor-settings-preview"><code>const NyouOS = { theme: 'accent' };</code></pre>`;
        ...
        fontHost.appendChild(NyouUI.Select({
...
const DeveloperCenterApiDocsApp = {
    ...
    init(windowId) {
        ...
        this.render();
        DeveloperCenterApp.notifyAiSkillSuggestion();
    ...
    render() {
        ...
        const subtitle = english ? 'NyouOS APIs available to sandboxed professional Apps.' : '专业 App 在沙箱中可使用的 NyouOS API。';
        const titleElement = document.getElementById(this.windowId)?.querySelector('.window-title');
        if (titleElement) titleElement.textContent = title;
        const downloadButton = DeveloperCenterApp.button(DeveloperCenterApp.text('downloadAiSkill'), 'primary', () => DeveloperCenterApp.downloadAiSkill(), 'Download');
        const page = document.createElement('main');
        page.className = 'dc-api-window-page';
        page.appendChild(DeveloperCenterApp.heading(title, subtitle, downloadButton));
        page.insertAdjacentHTML('beforeend', DeveloperCenterApp.apiDocsContent());
...
const DeveloperCenterController = {
    appEntry: {
        ...
        icon: 'Theme/Icon/App_icon/developer_center.webp', isNative: true, isPWA: false
    },
    initialized: false,
