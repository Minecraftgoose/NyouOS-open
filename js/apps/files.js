...
const FilesApp = {
    ...
    SYSTEM_ROOT_FOLDERS: ['desktop', 'documents', 'pictures', 'music', 'downloads', 'recycle'],
    ...
    _isClosing: false,
    _renderGeneration: 0,
    _mediaCoverCache: new Map(),
    _mediaCoverLoads: new Set(),
    ...
    init(windowId, data = {}) {
        ...
        // 监听隐私页“文件上传”开关：动态显示/隐藏上传入口与外部拖拽导入
    ...
    getQuickAccessItems() {
        return [
            ...
            { id: 'music', label: t('files.music'), icon: 'Music' },
    ...
    render() {
        ...
        if (typeof NyouWindow === 'undefined' || typeof NyouWindow.mount !== 'function') {
            console.error('[FilesApp] NyouWindow framework is not loaded');
            return;
        }
    ...
    renderContentPage(pageEl) {
        ...
        // 使用 NyouUI.NavigationBar 创建工具栏
        const searchBox = NyouUI.SearchBox({
        ...
        if (externalEnabled) {
            ...
            const uploadBtn = NyouUI.IconButton
                ? NyouUI.IconButton({
                    icon: 'Upload',
        ...
        const toolbar = NyouUI.NavigationBar({
            showBack: true,
            showForward: true,
        ...
        toolbar.className = 'files-toolbar Nyou-navbar';
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .files-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 16px; align-items: stretch; }
            .file-item { display: flex; flex-direction: column; align-items: center; gap: 8px; min-width: 0; padding: 12px 8px; box-sizing: border-box; border-radius: var(--radius-md); cursor: pointer; transition: background var(--transition-fast); }
            ...
            .file-item-music-preview { display: grid; place-items: center; overflow: hidden; background: linear-gradient(135deg, #5e5ce6 0%, #64d2ff 52%, #e0e7ff 100%); }
            .file-item-music-preview img { width: 30px !important; height: 30px !important; filter: brightness(0) invert(1); opacity: .94; }
            .file-item.selected .file-item-image-preview { box-shadow: 0 8px 22px rgba(0,0,0,0.22); }
            .file-item > .file-item-name { display: block; width: 100%; min-width: 0; height: 1.2em; max-height: 1.2em; overflow: hidden; color: inherit; font-size: 13px; line-height: 1.2; text-align: center; white-space: nowrap; text-overflow: ellipsis; }
            .file-item.selected > .file-item-name { height: auto; max-height: none; overflow: visible; white-space: normal; overflow-wrap: anywhere; word-break: break-word; text-overflow: clip; }
    ...
    bindEvents() {
        ...
        // 记录这一次操作实际来自鼠标还是触摸，兼容同时具备两者的设备。
        filesList.addEventListener('pointerdown', (e) => {
            if (e.isPrimary === false || (typeof e.button === 'number' && e.button !== 0)) return;
            const item = e.target.closest('.file-item');
            this._lastItemPointer = item ? {
                id: item.dataset.id,
        ...
        filesList.addEventListener('click', (e) => {
            ...
            if (item) {
                ...
                const pointer = this._lastItemPointer;
                if (pointer?.pointerType === 'touch' && pointer.id === item.dataset.id) {
                    this.openItemById(item.dataset.id);
                }
            }
        }, listenerOptions);
        ...
        filesList.addEventListener('dblclick', (e) => {
            ...
            if (!item) return;
            const pointer = this._lastItemPointer;
            if (pointer?.pointerType === 'touch' && pointer.id === item.dataset.id) return;
            this.openItemById(item.dataset.id);
        }, listenerOptions);
    ...
    copyToClipboard() {
        ...
        NyouUI.Toast({ title: t('files.title'), message: t('files.copied', {count: items.length}), type: 'info' });
    },
    ...
    cutToClipboard() {
        ...
        // 添加剪切样式
        this.renderFileList();
        NyouUI.Toast({ title: t('files.title'), message: t('files.cut-done', {count: items.length}), type: 'info' });
    },
    ...
    pasteFromClipboard() {
        ...
        if (pastedCount > 0) {
            NyouUI.Toast({ title: t('files.title'), message: t('files.pasted', {count: pastedCount}), type: 'success' });
        }
    },
    ...
    renameNode(id) {
        ...
        const applyRename = (nextName) => {
            ...
            this.renderFileList();
            NyouUI.Toast({ title: t('files.rename-title'), message: t('files.renamed', {name: nextName}), type: 'success' });
        };
    ...
    async deleteNodePermanent(id) {
        ...
        try {
            if (globalThis.NyouOSStorage) await NyouOSStorage.purgeNodes([node]);
        } catch (error) {
            console.error('[FilesApp] Failed to permanently delete payload', error);
            NyouUI.Toast({ title: t('files.recycle'), message: I18n.currentLang === 'en' ? 'The local payload could not be deleted. The file was kept.' : '无法删除浏览器本地数据，文件已保留。', type: 'error' });
            return;
        }
    ...
    async deleteSelectedPermanent() {
        ...
        try {
            if (globalThis.NyouOSStorage) await NyouOSStorage.purgeNodes(selectedNodes);
        } catch (error) {
            console.error('[FilesApp] Failed to permanently delete selected payloads', error);
            NyouUI.Toast({ title: t('files.recycle'), message: I18n.currentLang === 'en' ? 'Some local payloads could not be deleted. The files were kept.' : '部分浏览器本地数据无法删除，文件已保留。', type: 'error' });
            return;
        }
        ...
        NyouUI.Toast({ title: t('files.recycle'), message: t('files.deleted-permanent', {count: count}), type: 'info' });
    },
    ...
    async emptyRecycle() {
        ...
        try {
            if (globalThis.NyouOSStorage) await NyouOSStorage.purgeNodes(nodes);
        } catch (error) {
            console.error('[FilesApp] Failed to empty recycle bin payloads', error);
            NyouUI.Toast({ title: t('files.recycle'), message: I18n.currentLang === 'en' ? 'The recycle bin could not be emptied because local data deletion failed.' : '浏览器本地数据删除失败，无法清空回收站。', type: 'error' });
            return;
        }
        recycle.children = [];
        State.updateFS(State.fs);
    ...
    updateSidebarActive() {
        ...
        this.container.querySelectorAll('.fw-nav-item').forEach(item => {
            ...
            if (icon && icon.dataset.iconName && typeof NyouWindow !== 'undefined' && NyouWindow._iconPath) {
                const iconPath = NyouWindow._iconPath(icon.dataset.iconName, isActive ? 'fill' : 'stroke');
    ...
    isAudioNode(node) {
        if (!node || node.type !== 'file') return false;
        if (String(node.mime || '').toLowerCase().startsWith('audio/')) return true;
        return /\.(?:mp3|m4a|aac|ogg|oga|wav|flac|opus|webm)$/i.test(String(node.name || ''));
    },

    mediaRecordIdForNode(node) {
        if (!node) return '';
        return String(node.mediaRecordId || (node.encoding === 'media-local-cache' ? `fs-${node.id}` : ''));
    },

    queueMediaCover(node) {
        const recordId = this.mediaRecordIdForNode(node);
        if (!recordId || this._mediaCoverCache.has(recordId) || this._mediaCoverLoads.has(recordId)) return;
        if (!globalThis.MediaApp || typeof MediaApp.loadCoverForNode !== 'function') return;
        this._mediaCoverLoads.add(recordId);
        Promise.resolve(MediaApp.loadCoverForNode(node)).then((url) => {
            this._mediaCoverCache.set(recordId, url || '');
            if (url && this.container && !this.isSearching) this.renderFileList();
        }).catch(() => {
            this._mediaCoverCache.set(recordId, '');
        }).finally(() => {
            this._mediaCoverLoads.delete(recordId);
        });
    },
    ...
    renderFileIconHTML(node) {
        if (this.isImageNode(node)) {
            ...
            if (preview) {
                return `<img class="file-item-image-preview" src="${preview}" alt="${node.name}" draggable="false">`;
            }
        }
        if (this.isAudioNode(node)) {
            const recordId = this.mediaRecordIdForNode(node);
            const liveCover = globalThis.MediaApp && typeof MediaApp.peekCoverForNode === 'function'
                ? MediaApp.peekCoverForNode(node)
                : '';
            const cover = liveCover || this._mediaCoverCache.get(recordId) || '';
            if (!this._mediaCoverCache.has(recordId)) this.queueMediaCover(node);
            if (cover) {
                return `<img class="file-item-image-preview" src="${cover}" alt="${node.name}" draggable="false">`;
            }
            return `<span class="file-item-image-preview file-item-music-preview" role="img" aria-label="${node.name}"><img src="Theme/Icon/Symbol_icon/stroke/Music.svg" alt="" draggable="false"></span>`;
        }
        if (node.type === 'app') {
            const desktopIcon = typeof Desktop !== 'undefined' && typeof Desktop.getDesktopIcon === 'function'
                ? Desktop.getDesktopIcon(node)
                : '';
            const appIcon = desktopIcon || WindowManager.getAppConfig?.(node.appId)?.icon || 'Theme/Icon/App_icon/app_error.webp';
            return `<img src="${appIcon}" alt="${node.name}" draggable="false">`;
        }
        if (node.type === 'file' && String(node.name || '').toLowerCase().endsWith('.fap')) {
            return `<img src="Theme/Icon/App_icon/App_package.webp" alt="${node.name}" draggable="false">`;
        }
        const icon = node.type === 'folder'
            ? 'Theme/Icon/Symbol_icon/stroke/Folder.svg'
            : 'Theme/Icon/Symbol_icon/stroke/File.svg';
        return `<img src="${icon}" alt="${node.name}" draggable="false">`;
    },
    ...
    renderFileList() {
        ...
        displayChildren.forEach(node => {
            ...
            globalThis.TouchDragBridge?.bind(item, { signal: this._listController?.signal });

            // Allow both NyouOS nodes and external OS files to target a folder item.
            if (node.type === 'folder') {
                const externalEnabled = () => !!(window.FileImport && typeof FileImport.enabled === 'function' && FileImport.enabled());
                const hasInternalPayload = (dataTransfer) => Array.from(dataTransfer?.types || [])
                    .map((value) => String(value || ''))
                    .includes('application/Nyou-file');
    ...
    openNodeWithDefaultApp(node, launchOptions = {}) {
        if (!node || node.type !== 'file' || typeof WindowManager === 'undefined') return false;

        if (this.isImageNode(node)) {
            WindowManager.openApp('photos', { fileId: node.id }, launchOptions);
            return true;
        }
        ...
        if (ext === 'fap') {
            WindowManager.openApp('app-installer', { fileId: node.id }, launchOptions);
            return true;
        }
        if (ext === 'mp3') {
            WindowManager.openApp('media', { fileId: node.id }, launchOptions);
            return true;
        }
        if (ext === 'txt' || ext === 'md') {
            WindowManager.openApp('notes', { fileId: node.id }, launchOptions);
            return true;
        }
        return false;
    },
    ...
    bindDragDropEvents(dropZone, highlightTarget = dropZone, signal = this._viewController?.signal) {
        ...
        const parseDropData = (dataTransfer) => {
            const fileData = dataTransfer?.getData('application/Nyou-file');
            if (fileData) {
                try {
        ...
        dropZone.addEventListener('dragover', (e) => {
            ...
            const external = externalEnabled() && !internal && hasExternalFiles(e.dataTransfer);
            if (!internal && !external) return;
            e.preventDefault();
            e.stopPropagation();
    ...
    handleFileDrop(data, destinationNode = this.currentNode) {
        ...
        if (movedCount === 0) {
            if (hasMoveError) {
                NyouUI.Toast({ title: t('files.title'), message: t('files.move-error'), type: 'error' });
            }
            return;
        }
        ...
        if (movedCount === 1) {
            NyouUI.Toast({ title: t('files.title'), message: t('files.moved', {name: firstMovedName}), type: 'success' });
        } else {
            NyouUI.Toast({ title: t('files.title'), message: t('files.items', {count: movedCount}), type: 'success' });
        }
    },
