/**
 * Camera App
 * Native NyouOS webcam capture app.
 */
const CameraApp = {
    windowId: null,
    container: null,
            ...
            <div class="camera-app camera-mode-${this.mode}">
                <div class="camera-stage">
                    ...
                    <div class="camera-permission">
                        <img src="Theme/Icon/App_icon/camera.webp" alt="">
                        <h2>${t('camera.permission-title')}</h2>
                        <p>${t('camera.permission-desc')}</p>
                        <button class="nyou-btn nyou-btn-primary nyou-btn-large camera-start" type="button"><span class="nyou-btn-text">${t('camera.start')}</span></button>
                    </div>
    ...
    capturePhoto() {
        ...
        canvas.toBlob(blob => {
            ...
            const capture = this.createCapture({
                ...
                filename: `Nyou-camera-${this.getTimestamp()}.png`
            });
    ...
    finishRecording() {
        ...
        const capture = this.createCapture({
            ...
            blob,
            url: URL.createObjectURL(blob),
            filename: `Nyou-camera-${this.getTimestamp()}.webm`
        });
        this.addCapture(capture);
    ...
    async setCurrentAsWallpaper() {
        ...
        if (typeof State !== 'undefined' && typeof Desktop !== 'undefined') {
            try {
                ...
                NyouUI.Toast({ title: t('camera.wallpaper'), message: t('settings.custom-wallpaper-fail'), type: 'error' });
            }
        }
    },
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .camera-shutter {
                ...
                backdrop-filter: blur(var(--Nyou-material-blur-light, 20px)) saturate(160%);
                -webkit-backdrop-filter: blur(var(--Nyou-material-blur-light, 20px)) saturate(160%);
            }
