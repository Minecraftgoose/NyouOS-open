/**
 * Photos App
 * NyouWindow-based photo library for Bing wallpapers, local photos and favorites.
 */
(function () {
    'use strict';

    const PHOTO_SETTINGS_KEY = 'NyouOS.photos.settings.v1';
    const PHOTO_BING_CACHE_KEY = 'NyouOS.photos.bingCache.v1';
    const PHOTO_LOCAL_IMAGE_PREFIX = 'NyouOS.photos.localImage.v1.';
    const PHOTO_IDB_NAME = 'NyouOS.photos.cache.v1';
    ...
    function safeImageName(name, fallback = 'photo.webp') {
    ...
    const PhotosDataStore = {
        ...
        async storeLocalImage(id, source, meta = {}) {
            ...
            const record = {
                ...
                ownerAppId: meta.ownerAppId === 'files' ? 'files' : 'photos',
        ...
        async importFiles(files) {
            ...
            try {
                for (const file of input) {
                    ...
                    const node = {
                        id: nodeId,
                        name: uniqueNameIn(folders.local, file.name || 'photo.webp'),
                        type: 'file',
                        content: cacheKey,
                        cacheKey,
                        cacheId: nodeId,
            ...
            if (capacityExceeded && globalThis.NyouUI?.Toast) {
                NyouUI.Toast({
                    title: typeof t === 'function' ? t('photos.title') : '照片',
        ...
        addLocalFavorite(nodeId) {
            ...
            const node = {
                ...
                name: uniqueNameIn(folders.favorites, source.name || 'photo.webp'),
                type: 'file',
                content: nodeId,
                refId: nodeId,
        ...
        addBingFavorite(item) {
            ...
            if (existing) return existing;
            const baseName = safeImageName(`${item.title || text('bing')}.jpg`, 'bing-wallpaper.webp');
            const node = {
    ...
    const PhotosApp = {
        ...
        render() {
            ...
            if (typeof NyouWindow === 'undefined' || typeof NyouWindow.mount !== 'function') {
                console.error('[PhotosApp] NyouWindow framework is not loaded');
                return;
            }
        ...
        renderSettingsPage(pageEl) {
            ...
            if (storageHost && window.NyouUI && NyouUI.Select) {
                storageHost.appendChild(NyouUI.Select({
            ...
            if (sourceHost && window.NyouUI && NyouUI.Select) {
                sourceHost.appendChild(NyouUI.Select({
        ...
        mountSettingsButton(pageEl, hostName, label, icon, onClick, variant = 'secondary') {
            ...
            if (!host) return;
            if (window.NyouUI && NyouUI.Button) {
                host.appendChild(NyouUI.Button({ text: label, icon, variant, onClick }));
                return;
            }
            const btn = document.createElement('button');
            btn.className = `nyou-btn nyou-btn-${variant}`;
            btn.type = 'button';
        ...
        async importFiles(files) {
            ...
            try {
                ...
                if (imported.length) this.toast(this.tr('imported', { count: imported.length }), 'success');
                if (imported.length && this.frame) {
        ...
        confirmClearImported() {
            ...
            if (window.NyouUI && NyouUI.Dialog) {
                NyouUI.Dialog({
        ...
        renderToolbar() {
            const isV2 = document.body.classList.contains('Nyou-v2');
        ...
        toast(message, type = 'info') {
            if (window.NyouUI && NyouUI.Toast) {
                NyouUI.Toast({ title: this.tr('bing'), message, type });
