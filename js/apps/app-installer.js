/** Unified, non-executing installer for Nyou .fap packages. */
const AppInstallerApp = {
    ...
    init(windowId, data = null) {
        ...
        if (title) title.textContent = this._text('Nyou App安装工具', 'Nyou App Installer');
        this.openData(data || {});
        return true;
    },
    ...
    async openData(data = {}) {
        ...
        this.existingApp = null;
        this.installedApp = null;
        this.installSource = data?.source === 'developer-center' ? 'developer-center' : 'external';
    ...
    _button(label, variant, action, disabled = false) {
        return `<button type="button" class="nyou-btn nyou-btn-${variant} nyou-btn-medium" data-installer-action="${action}"${disabled ? ' disabled' : ''}><span class="nyou-btn-text">${this._esc(label)}</span></button>`;
    },
    ...
    _renderIntro() {
        ...
        this._shell(`
            <div class="app-installer-summary">
                <img class="app-installer-app-icon" src="${this._esc(app.icon || 'Theme/Icon/App_icon/created_app.webp')}" alt="">
    ...
    async _runSafetyCheck() {
        const token = ++this._runToken;
        this._shell(`<div class="app-installer-state"><span class="app-installer-spinner" aria-hidden="true"></span><h2>${this._text('正在进行安全检查', 'Running safety inspection')}</h2><p>${this._text('Nyou 正在静态检查安装包，不会运行其中的任何代码。', 'Nyou is inspecting the package statically. No App code will run.')}</p></div>`);
    ...
    async _saveImportedApp(imported) {
        const next = { ...imported };
        const packageProjectId = next.projectId;
        const existing = this.existingApp;
        const importedThroughDeveloperCenter = this.installSource === 'developer-center';
        const canExposeSource = importedThroughDeveloperCenter && next.allowUnpack === true;
        const existingProject = existing?.projectId
            ? await DeveloperCenterStore.get('projects', existing.projectId)
            : null;
        if (existing) {
            next.id = existing.id;
        ...
        next.developerCenterVisible = importedThroughDeveloperCenter;
        if (canExposeSource && existingProject && existingProject.importedFromFap !== true) {
            // Reinstalling an App authored on this device must not overwrite or
            // duplicate its original editable project.
            next.projectId = existingProject.id;
        } else if (canExposeSource) {
            const project = await DeveloperCenterStore.saveProject({
                id: existingProject?.importedFromFap === true ? existingProject.id : packageProjectId,
                name: next.name,
                type: next.type,
                title: next.title,
                ...
                css: next.css,
                js: next.js,
                forceNyouUI: next.forceNyouUI === true,
        ...
        } else if (existingProject?.importedFromFap === true) {
            await DeveloperCenterStore.remove('projects', existingProject.id);
            next.projectId = packageProjectId;
    ...
    _renderComplete() {
        ...
        this._shell(`
            <div class="app-installer-complete">
                <img class="app-installer-status-icon" src="Theme/Icon/Symbol_icon/colour/Installer Success.svg" alt="" aria-hidden="true">
                <h1>${replacing ? this._text('覆盖安装完成', 'Replacement complete') : this._text('安装完成', 'Installation complete')}</h1>
                <p>${replacing ? this._text(`${app.name} 已成功更新。`, `${app.name} was updated successfully.`) : this._text(`${app.name} 已成功安装到 Nyou。`, `${app.name} was installed successfully.`)}</p>
    ...
    _renderBlocked(reason) {
        const messages = this._english() ? {
            ...
            malicious: 'Potentially malicious code was detected. Nyou refused to install this App.',
            api: 'Potentially high-risk API requests were detected. Nyou refused to install this App.',
            version: 'This package was created for an unsupported Nyou App package version.',
            metadata: 'The package can be read, but required Nyou App information is missing or invalid.',
            ...
            malicious: '疑似含有潜在恶意代码，Nyou 已拒绝安装该 App。',
            api: '疑似包含高危 API 请求，Nyou 已拒绝安装该 App。',
            version: '该安装包使用了当前 Nyou 不支持的 App 安装包版本。',
            metadata: '安装包可以正常读取，但缺少必要的 Nyou App 信息或信息无效。',
        ...
        this._shell(`
            <div class="app-installer-blocked">
                <img class="app-installer-status-icon" src="Theme/Icon/Symbol_icon/colour/Installer Error.svg" alt="" aria-hidden="true">
    ...
    _packageErrorReason(error) {
        ...
        if (message.includes('unsupported Nyou app package') || message.includes('version')) return 'version';
