...
const NotesApp = {
    ...
    renderSettingsPage(pageEl) {
        ...
        if (locationHost && window.NyouUI && typeof NyouUI.Select === 'function') {
            locationHost.appendChild(NyouUI.Select({
        ...
        if (autosaveHost && window.NyouUI && typeof NyouUI.Toggle === 'function') {
            autosaveHost.appendChild(NyouUI.Toggle({
                checked: autosave,
    ...
    mountToolbarSelect(host, config = {}) {
        if (!host || !window.NyouUI || typeof NyouUI.Select !== 'function') return null;
        const select = NyouUI.Select({
        ...
        if (config.title) {
            ...
            const trigger = select.querySelector('.Nyou-select-trigger');
            if (trigger) {
    ...
    playNewNoteReveal(button, onReady) {
        ...
        setTimeout(showEditor, this.getNyouEasedDelay(revealDuration, 0.6));
        animation.finished.then(finish).catch(finish);
    },

    getNyouEasedDelay(duration, progress) {
    ...
    revealNote(fileId) {
        ...
        setTimeout(() => {
            ...
            if (filesComponent?.navigateToId) {
                filesComponent.navigateToId(item.parent.id, fileId);
            }
        }, 350);
    },
    ...
    askDelete() {
        if (!window.NyouUI || !NyouUI.Dialog) return Promise.resolve(confirm(this.tr('deleteBody')));
        return new Promise(resolve => {
            NyouUI.Dialog({
                type: 'warning',
    ...
    askForText(title, value) {
        if (window.NyouUI && NyouUI.InputDialog) {
            return new Promise(resolve => {
                NyouUI.InputDialog({
                    title,
                    defaultValue: value,
    ...
    toast(message) {
        if (window.NyouUI && NyouUI.Toast) NyouUI.Toast({ title: this.tr('appTitle'), message, type: 'info' });
