...
const ClockApp = {
    ...
    render() {
        ...
        if (typeof NyouWindow === 'undefined' || typeof NyouWindow.mount !== 'function') {
            console.error('[ClockApp] NyouWindow framework is not loaded');
            return;
        }
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .calendar-add-btn.nyou-btn { min-height: 36px; padding: 0 18px; border: none; border-radius: 999px; cursor: pointer; font-size: 13px; }
            ...
            .calendar-event-action.nyou-btn { min-width: 68px; min-height: 32px; padding: 0 14px; border-radius: 999px; font-size: 12px; cursor: pointer; }
            .calendar-event-action .nyou-btn-text { color: inherit !important; }
            .calendar-event-complete.nyou-btn { color: var(--accent) !important; }
            .calendar-event.completed .calendar-event-complete.nyou-btn { color: var(--text-secondary) !important; }
            .calendar-empty { text-align: center; padding: 32px; color: var(--text-tertiary); }
            .clock-event-modal.Nyou-modal-overlay { background: rgba(0, 0, 0, 0.42); backdrop-filter: blur(24px) saturate(125%); -webkit-backdrop-filter: blur(24px) saturate(125%); }
            .clock-event-modal .Nyou-modal { overflow: hidden; }
            .clock-event-modal .Nyou-modal-header,
            .clock-event-modal .Nyou-modal-content { background: var(--bg-primary); }
            .clock-event-modal .Nyou-modal-footer { gap: 10px; padding: 16px 24px 22px; background: var(--bg-secondary); border-top-color: var(--border-color); }
            .clock-event-modal .Nyou-modal-footer .nyou-btn { min-width: 76px; min-height: 34px; padding: 0 18px; border-radius: 999px; font-size: 13px; }
            body.Nyou-v2 .clock-event-modal .Nyou-modal-header,
            body.Nyou-v2 .clock-event-modal .Nyou-modal-content { background: rgba(255, 255, 255, 0.38); }
            body.Nyou-v2 .clock-event-modal .Nyou-modal-footer { background: rgba(255, 255, 255, 0.52); border-top-color: var(--v2-glass-border); }
            body.dark-mode .clock-event-modal .Nyou-modal-header,
            body.dark-mode .clock-event-modal .Nyou-modal-content { background: var(--bg-primary); }
            body.dark-mode .clock-event-modal .Nyou-modal-footer { background: var(--bg-secondary); }
            body.Nyou-v2.dark-mode .clock-event-modal .Nyou-modal-header,
            body.Nyou-v2.dark-mode .clock-event-modal .Nyou-modal-content { background: rgba(32, 32, 36, 0.58); }
            body.Nyou-v2.dark-mode .clock-event-modal .Nyou-modal-footer { background: rgba(24, 24, 28, 0.72); border-top-color: var(--v2-glass-border-dark); }
            ...
            body.Nyou-v2 .clock-content .timer-btn.nyou-btn { min-width: 148px; }
            body:not(.dark-mode) .clock-content .timer-btn.nyou-btn,
            body:not(.dark-mode) .clock-content .timer-btn.nyou-btn .nyou-btn-text {
            ...
            body.dark-mode .clock-content .timer-btn.nyou-btn,
            body.dark-mode .clock-content .timer-btn.nyou-btn .nyou-btn-text {
    ...
    renderTimer() {
        ...
        return `
            ...
            <div class="timer-controls">
                ${this.timerInterval ? `
                    <button class="nyou-btn nyou-btn-secondary nyou-btn-large timer-btn" id="timer-pause">${t('clock.pause')}</button>
                    <button class="nyou-btn nyou-btn-secondary nyou-btn-large timer-btn" id="timer-reset">${t('clock.reset')}</button>
                ` : `
                    <button class="nyou-btn nyou-btn-primary nyou-btn-large timer-btn" id="timer-start">${this.timerRemaining > 0 ? t('clock.continue') : t('clock.start')}</button>
                    ${this.timerRemaining > 0 ? `<button class="nyou-btn nyou-btn-secondary nyou-btn-large timer-btn" id="timer-reset">${t('clock.reset')}</button>` : ''}
                `}
            </div>
        `;
    },
        ...
        return `
            ...
            <div class="stopwatch-controls">
                ${this.stopwatchInterval ? `
                    <button class="nyou-btn nyou-btn-secondary nyou-btn-large timer-btn" id="stopwatch-lap">${t('clock.lap')}</button>
                    <button class="nyou-btn nyou-btn-primary nyou-btn-large timer-btn" id="stopwatch-pause">${t('clock.pause')}</button>
                ` : `
                    <button class="nyou-btn nyou-btn-primary nyou-btn-large timer-btn" id="stopwatch-start">${this.stopwatchTime > 0 ? t('clock.continue') : t('clock.start')}</button>
                    ${this.stopwatchTime > 0 ? `<button class="nyou-btn nyou-btn-secondary nyou-btn-large timer-btn" id="stopwatch-reset">${t('clock.reset')}</button>` : ''}
                `}
            </div>
            ...
            <div class="calendar-container">
                ...
                <div class="calendar-events">
                    <div class="calendar-events-header">
                        ...
                        ${selectedDateStr ? `<button type="button" class="calendar-add-btn nyou-btn nyou-btn-primary nyou-btn-medium" id="calendar-add-event"><span class="nyou-btn-text">${t('clock.add-event')}</span></button>` : ''}
                    </div>
                    ${selectedDateStr ? (sortedDayEvents.length > 0 ? `<div class="calendar-events-list">${sortedDayEvents.map(event => `
                        <div class="calendar-event ${event.completed ? 'completed' : ''}" data-id="${this.escapeHtml(event.id)}">
                            ...
                            <div class="calendar-event-actions">
                                <button type="button" class="calendar-event-action calendar-event-complete nyou-btn nyou-btn-secondary nyou-btn-small" data-id="${this.escapeHtml(event.id)}"><span class="nyou-btn-text">${event.completed ? t('clock.undo-complete') : t('clock.complete')}</span></button>
                                <button type="button" class="calendar-event-action calendar-event-edit nyou-btn nyou-btn-secondary nyou-btn-small" data-id="${this.escapeHtml(event.id)}"><span class="nyou-btn-text">${t('clock.edit')}</span></button>
                            </div>
                        </div>
    ...
    openEventDialog(event = null) {
        ...
        const saveEvent = () => {
            ...
            if (isEdit) {
                ...
                NyouUI.Toast({ title: t('clock.event-updated'), message: `${title} - ${normalizedTime}`, type: 'success' });
            } else {
                const newEvent = {
                    id: `event-${Date.now()}`,
                ...
                this.saveData();
                this.renderKeepingClockScroll(dialogScrollTop);
                NyouUI.Toast({ title: t('clock.event-added'), message: `${title} - ${normalizedTime}`, type: 'success' });
            }
...
const ClockReminderService = {
    ...
    showReminderPopup(notificationId, notification) {
        if (State.view !== 'desktop' || typeof NyouUI === 'undefined' || !NyouUI.Toast) return;
        if (this.toastHandles.has(notificationId)) return;

        const toastHandle = NyouUI.Toast({
