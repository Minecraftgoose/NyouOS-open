...
const SettingsApp = {
    ...
    aboutChangelog: [
        { date: '2026-08-18', version: '27.0', commits: [
            { hash: 'local', title: 'NyouOS On Web 27.0 正式发布，基于 FluentOS On Web 二次开发，闭源专有软件。' },
            { hash: 'nyou-001', title: 'Fingo AI 全面升级为 SurfAi，接入 Anges API，支持云端智能对话。' },
            { hash: 'nyou-002', title: '全新设置界面，采用 NyouWindow 框架，统一 Fluent Design 设计语言。' },
            { hash: 'nyou-003', title: '修复开机 OOBE 卡死、全局动画丢失、壁纸路径错误、锁屏白屏等大量问题。' },
            { hash: 'nyou-004', title: '优化锁屏到桌面的非线性景深过渡动画，支持 PIN 登录流程。' }
        ] },
        { date: '2026-08-16', version: '27.0 Beta', commits: [
            { hash: 'nyou-005', title: '项目从 MIT 开源转为闭源，© 2025-2026 KevinAnanda 保留所有权利。' },
            { hash: 'nyou-006', title: '多 AI 协作开发框架搭建，支持豆包/WorkBuddy/TRAE/Codex 协同工作。' }
        ] },
        { date: '2026-08-01', version: '27.0 Alpha', commits: [
            { hash: 'nyou-007', title: '基于 FluentOS On Web 青山湖版本启动 NyouOS 二次开发。' }
        ] }
    ],
    ...
    _storageSnapshotPromise: null,
    _storageSnapshotToken: 0,
    ...
    getAppSize(appId, isPWA = false) {
        ...
        if (!app || !globalThis.NyouOSStorage) return 0;
        return NyouOSStorage.getAppSize({ ...app, isPWA: isPWA === true || app.isPWA === true });
    },

    prepareStorageSnapshot() {
        const token = ++this._storageSnapshotToken;
        this._lastStorageSnapshot = null;
        if (!globalThis.NyouOSStorage) {
            this._storageSnapshotPromise = Promise.resolve(null);
            return;
        }
        const apps = this.getStorageApps();
        this._storageSnapshotPromise = NyouOSStorage.analyze(apps, { force: true })
            .then((snapshot) => {
                if (token === this._storageSnapshotToken) this._lastStorageSnapshot = snapshot;
                return token === this._storageSnapshotToken ? snapshot : null;
            })
            .catch((error) => {
                if (token === this._storageSnapshotToken) console.error('[SettingsApp] Storage analysis failed', error);
                return null;
            });
    },

    getPreparedStorageSnapshot() {
        if (this._lastStorageSnapshot) return Promise.resolve(this._lastStorageSnapshot);
        return this._storageSnapshotPromise || Promise.resolve(null);
    },
    ...
    getNavActivePageId(pageId = this.currentPage) {
        return {
            'personalization-advanced': 'personalization',
            'app-detail': 'applications',
            'about-changelog': 'about'
        }[pageId] || pageId;
    },

    createSecondaryBackBar(label, onBack) {
        const bar = document.createElement('div');
        bar.className = 'settings-secondary-topbar';
        bar.setAttribute('data-fw-scroll-bounce-fixed', '');
        bar.appendChild(NyouUI.Button({
            text: label,
            icon: 'Arrow Left',
            variant: 'secondary',
            className: 'settings-secondary-back-button',
            onClick: onBack
        }));
        return bar;
    },
    ...
    beforeClose() {
        this._storageSnapshotToken += 1;
        this._storageSnapshotPromise = null;
        this._lastStorageSnapshot = null;
        this._stateUnsubscribers.splice(0).forEach((unsubscribe) => unsubscribe());
    ...
    render() {
        ...
        if (typeof NyouWindow === 'undefined' || typeof NyouWindow.mount !== 'function') {
            console.error('[SettingsApp] NyouWindow framework is not loaded');
            return;
        }

        this.frame = NyouWindow.mount({
            container: this.container,
            items: this.getPages().map(p => ({ id: p.id, label: t(p.label), icon: p.icon })),
            activeId: this.currentPage,
            getNavActiveId: (pageId) => this.getNavActivePageId(pageId),
            ...
            onNavigate: (pageId, pageEl) => {
                // NyouWindow reuses the same page element. Clear secondary-page
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .settings-content { flex: 1; overflow-y: auto; padding: 40px; min-height: 100%; box-sizing: border-box; }
            ...
            body.fluent-v2.window-blur-disabled .settings-content > div:not(.fluent-setting-item):not(.settings-recommend-item):not(.settings-recent-item):not(.network-hero-card):not(.network-option-item):not(.network-expand-panel):not(.wallpaper-item):not(.settings-overview-wallpaper):not(.settings-secondary-topbar),
            ...
            .settings-content.settings-secondary-page {
                overflow: visible;
            }

            .settings-secondary-topbar {
                --settings-secondary-bar-solid: #fff;
                --settings-secondary-bar-90: rgba(255, 255, 255, .9);
                --settings-secondary-bar-45: rgba(255, 255, 255, .45);
                position: sticky;
                top: 0;
                z-index: 30;
                display: flex;
            ...
            .dark-mode .settings-secondary-topbar {
                --settings-secondary-bar-solid: #000;
                --settings-secondary-bar-90: rgba(0, 0, 0, .9);
                --settings-secondary-bar-45: rgba(0, 0, 0, .45);
            }

            .settings-secondary-topbar::before {
                content: '';
                position: absolute;
                inset: -10px -10px auto;
                z-index: 1;
                height: 20px;
                background: var(--settings-secondary-bar-solid);
                pointer-events: none;
            }

            .settings-secondary-topbar::after {
                content: '';
                position: absolute;
                inset: -10px -10px 0;
                z-index: 0;
                background: linear-gradient(
                    to bottom,
                    var(--settings-secondary-bar-solid) 0%,
                    var(--settings-secondary-bar-solid) 34%,
                    var(--settings-secondary-bar-90) 52%,
                    var(--settings-secondary-bar-45) 76%,
                    transparent 100%
                );
                pointer-events: none;
            }

            .settings-secondary-topbar .settings-secondary-back-button {
                position: relative;
                z-index: 2;
                pointer-events: auto;
                flex: 0 0 auto;
            ...
            @property --material-preview-pan-y {
                syntax: '<percentage>';
                inherits: true;
                initial-value: 0%;
            }

            @keyframes materialPreviewWallpaperPan {
                0%, 100% {
                    --material-preview-pan-y: 0%;
                    animation-timing-function: cubic-bezier(0.45, 0, 0.55, 1);
                }
                50% {
                    --material-preview-pan-y: 100%;
                    animation-timing-function: cubic-bezier(0.45, 0, 0.55, 1);
                }
            }
            ...
            .material-preview {
                ...
                box-shadow: var(--shadow-md);
                animation: materialPreviewWallpaperPan 18s linear infinite;
            }
            ...
            .material-preview-wallpaper {
                ...
                background-position:
                    ...
                    center,
                    center,
                    center var(--material-preview-pan-y);
            }
            ...
            .material-preview-panel {
                ...
                border: none;
                background: transparent;
                backdrop-filter: blur(var(--fluent-material-blur, 40px)) saturate(145%);
                -webkit-backdrop-filter: blur(var(--fluent-material-blur, 40px)) saturate(145%);
                box-shadow: 0 18px 42px rgba(24, 68, 120, 0.18);
            }

            .material-preview-gaussian .material-preview-panel::after {
                content: '';
                position: absolute;
                inset: 0;
                z-index: 0;
            ...
            .material-preview-mica .material-preview-panel::before {
                ...
                background-position: center var(--material-preview-pan-y);
            ...
            .material-preview-mica .material-preview-panel::after {
                ...
                z-index: 0;
                background:
                    linear-gradient(135deg, rgba(255,255,255,0.34), rgba(225,232,242,0.12)),
                    rgba(243, 246, 250, 0.84);
                pointer-events: none;
            }
            ...
            .dark-mode .material-preview-gaussian .material-preview-panel::after {
            ...
            .dark-mode .material-preview-mica .material-preview-panel::before {
                filter: blur(24px) saturate(88%) brightness(0.74);
            }

            .dark-mode .material-preview-mica .material-preview-panel::after {
                background:
                    linear-gradient(135deg, rgba(64,69,80,0.22), rgba(0,0,0,0.20)),
                    rgba(28, 30, 35, 0.86);
            }
    ...
    renderOverview(container) {
        ...
        const wallpaper = typeof State.getResolvedWallpaper === 'function'
            ...
            : (State.settings.wallpaperDesktop || 'Theme/Picture/Fluent-2.webp');
        header.innerHTML = `
    ...
    getUserAvatarOptions() {
        return [
            'Theme/Profile_img/UserAva.webp',
    ...
    renderUser(container) {
        ...
        avatarActions.appendChild(NyouUI.Button({
        ...
        avatarActions.appendChild(NyouUI.Button({
            ...
            variant: 'secondary',
            onClick: () => {
                State.updateSettings({ userAvatar: 'Theme/Profile_img/UserAva.webp' });
                this.addRecentSetting(
                    t('settings.user.recent-avatar'),
        ...
        infoSection.appendChild(NyouUI.SettingItem({
            label: t('settings.user.name'),
            description: t('settings.user.lock-preview-desc'),
            control: nameInput
        }));

        const emailInput = NyouUI.Input({
            value: profile.email,
        ...
        `;
        resetCard.appendChild(NyouUI.Button({
    ...
    renderNetwork(container) {
        ...
        if (btExpanded) {
            ...
            if (btEnabled) {
                ...
                discoverRow.appendChild(NyouUI.Toggle({ checked: true }));
    ...
    renderTimeLanguage(container) {
        ...
        const langOptions = (I18n.languages || []).map(l => ({
            value: l.code,
            label: l.nativeName || l.name
        }));

        // 直接创建行，不用 NyouUI.SettingItem 包装，避免事件被拦截
        const langRow = document.createElement('div');
        langRow.className = 'fluent-setting-item';
        langRow.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:12px 16px;gap:16px;';
        const langLabel = document.createElement('div');
        langLabel.style.cssText = 'flex:1;';
        langLabel.innerHTML = `<div style="font-size:14px;font-weight:500;">${t('settings.language.display')}</div><div style="font-size:12px;opacity:0.7;margin-top:2px;">${t('settings.language.desc')}</div>`;
        const select = document.createElement('select');
        select.style.cssText = 'padding:8px 12px;border-radius:6px;border:1px solid var(--input-border, #ccc);background:var(--input-bg, #fff);color:var(--text-primary, #000);font-size:14px;min-width:160px;cursor:pointer;';
        (langOptions.length ? langOptions : [{ value: 'zh', label: '简体中文' }, { value: 'en', label: 'English' }]).forEach(opt => {
            const option = document.createElement('option');
            option.value = opt.value;
            option.textContent = opt.label;
            if (opt.value === (State.settings.language || I18n.currentLang || 'zh')) option.selected = true;
            select.appendChild(option);
        });
        select.addEventListener('change', (e) => {
            const v = e.target.value;
            I18n.setLanguage(v);
            if (State.settings) State.settings.language = v;
            State.save();
        ...
        // 辅助功能
        const a11ySection = this.createSection('辅助功能');

        // 开关类辅助功能
        const toggleOptions = [
            { key: 'invertMode', label: '反色模式', desc: '反转屏幕颜色，提高对比度，适合视力障碍用户' },
            { key: 'grayscaleMode', label: '灰度模式', desc: '整个屏幕变为黑白灰度显示' },
            { key: 'highContrastMode', label: '高对比度', desc: '增强屏幕元素之间的对比度' },
            { key: 'largeTextMode', label: '大字体', desc: '增大系统字体大小，便于阅读' }
        ];
        toggleOptions.forEach(opt => {
            a11ySection.appendChild(NyouUI.SettingItem({
                label: opt.label,
                description: opt.desc,
                control: NyouUI.Toggle({
                    checked: State.settings[opt.key] === true,
                    onChange: (v) => {
                        State.updateSettings({ [opt.key]: v });
                    }
                })
            }));
        });

        // 减少动画（复用 enableAnimation，反向逻辑）
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '减少动画',
            description: '禁用系统动画和过渡效果，提升响应速度',
            control: NyouUI.Toggle({
        ...
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '旁白',
            description: '朗读屏幕上的文字内容，鼠标悬停或聚焦元素时自动朗读',
            control: NyouUI.Toggle({
                checked: State.settings.screenReader === true,
                onChange: (v) => {
                    State.updateSettings({ screenReader: v });
                }
            })
        }));
        ...
        for (let r = 0.5; r <= 2; r += 0.25) {
            rateOptions.push({ value: r, label: r + 'x' });
        }
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '旁白语速',
            description: '调整旁白朗读的速度',
            control: NyouUI.Select({
                options: rateOptions,
                value: State.settings.screenReaderRate || 1,
                onChange: (v) => {
                    State.updateSettings({ screenReaderRate: parseFloat(v) });
                }
            })
        }));
        ...
        // 旁白音调
        const pitchOptions = [];
        for (let p = 0; p <= 2; p += 0.25) {
            pitchOptions.push({ value: p, label: p + 'x' });
        }
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '旁白音调',
            description: '调整旁白朗读的音调高低',
            control: NyouUI.Select({
                options: pitchOptions,
                value: State.settings.screenReaderPitch || 1,
                onChange: (v) => {
                    State.updateSettings({ screenReaderPitch: parseFloat(v) });
                }
            })
        }));
        ...
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '屏幕键盘',
            description: '聚焦输入框时自动显示虚拟键盘，可点击按键输入文字',
            control: NyouUI.Toggle({
                checked: State.settings.screenKeyboard === true,
                onChange: (v) => {
                    State.updateSettings({ screenKeyboard: v });
                }
            })
        }));
        ...
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '屏幕缩放',
            description: '调整整个界面的放大比例',
            control: NyouUI.Select({
                options: zoomOptions,
                value: State.settings.zoomLevel || 1,
                onChange: (v) => {
                    State.updateSettings({ zoomLevel: parseFloat(v) });
                }
            })
        }));
        ...
        const cbOptions = [
            { value: 'none', label: '关闭' },
            { value: 'protanopia', label: '红色弱 (Protanopia)' },
            { value: 'deuteranopia', label: '绿色弱 (Deuteranopia)' },
            { value: 'tritanopia', label: '蓝色弱 (Tritanopia)' }
        ];
        a11ySection.appendChild(NyouUI.SettingItem({
            label: '色弱模式',
            description: '针对红/绿/蓝色弱用户优化屏幕显示',
            control: NyouUI.Select({
                options: cbOptions,
                value: State.settings.colorBlindMode || 'none',
                onChange: (v) => {
                    State.updateSettings({ colorBlindMode: v });
                }
            })
        }));
    ...
    renderMultitask(container) {
        ...
        performanceSection.appendChild(NyouUI.SettingItem({
        ...
        section.appendChild(NyouUI.SettingItem({
    ...
    renderPrivacy(container) {
        ...
        securitySection.appendChild(NyouUI.SettingItem({
            label: t('settings.change-pin'),
            description: t('settings.change-pin.desc'),
            control: NyouUI.Button({
        ...
        privacySection.appendChild(NyouUI.SettingItem({
        ...
        privacySection.appendChild(NyouUI.SettingItem({
            label: t('settings.file-upload'),
            description: t('settings.file-upload-desc'),
            control: NyouUI.Toggle({
                checked: State.settings.enableExternalFileImport === true,
                onChange: (v) => {
                    State.updateSettings({ enableExternalFileImport: v });
                    this.addRecentSetting(t('settings.file-upload'), v ? t('settings.on') : t('settings.off'), 'privacy');
                    State.addNotification({
                        title: t('settings.privacy-title'),
                        message: v ? t('settings.file-upload-on') : t('settings.file-upload-off'),
                        type: 'info'
                    });
                }
            })
        ...
        const strictCspItem = NyouUI.SettingItem({
    ...
    renderFingo(container) {
        ...
        privSection.appendChild(NyouUI.SettingItem({
        ...
        // AI 模式
        const modeSection = this.createSection('AI 模式');
        const aiMode = State.settings.fingoAiMode || 'local';
        modeSection.appendChild(NyouUI.SettingItem({
            label: 'AI 引擎',
            description: '选择使用本地模型或自定义 API',
            control: NyouUI.Select({
                options: [
                    { value: 'local', label: 'SurfAi 本地模型（默认）' },
                    { value: 'custom', label: '自定义 API' }
                ],
                value: aiMode,
                onChange: (v) => {
                    State.updateSettings({ fingoAiMode: v });
                    if (v === 'local') State.updateSettings({ fingoCustomMode: false });
                    else State.updateSettings({ fingoCustomMode: true });
                    this.render();
                }
            })
        }));
        ...
        if (aiMode === 'custom') {
            ...
            const saveBtn = NyouUI.Button({
                text: '保存',
                variant: 'primary',
                onClick: () => {
                    const val = (keyInput.getValue ? keyInput.getValue() : keyInput.querySelector('input').value).trim();
                    if (val) {
                        State.updateSettings({ fingoApiKey: val, fingoApiStorageType: 'permanent-plain' });
                        NyouUI.Toast({ title: 'SurfAi', message: 'API Key 已保存', type: 'success' });
                    } else {
                        State.updateSettings({ fingoApiKey: '', fingoApiStorageType: 'none' });
                        NyouUI.Toast({ title: 'SurfAi', message: 'API Key 已清除', type: 'info' });
                    }
                    this.render();
                }
            });
            ...
            localInfo.className = 'fluent-setting-item';
            localInfo.innerHTML = '<div class="fluent-setting-item-info"><div class="fluent-setting-item-desc" style="font-size:13px;line-height:1.6;color:var(--text-secondary)">轻量本地模型已内置，零延迟运行。支持对话、问答、写作、翻译、代码等基础能力。</div></div>';
            modeSection.appendChild(localInfo);
        }
        ...
        aboutText.innerHTML = '<div class="fluent-setting-item-info"><div class="fluent-setting-item-desc" style="font-size:13px;line-height:1.8">SurfAi是NyouOS的智能助手，默认使用本地轻量模型，若要使用更高级的AI，请输入您的AI API。</div></div>';
    ...
    _showFingoCustomRiskDialog() {
        return new Promise((resolve) => {
            if (typeof NyouUI === 'undefined' || !NyouUI || typeof NyouUI.Dialog !== 'function') {
                resolve(false);
                return;
            }
        ...
        NyouUI.Dialog({
            ...
            onClose: async (result) => {
                ...
                try {
                    ...
                    NyouUI.Toast({ title: t('settings.fingo'), message: t('settings.fingo-perm-encrypted-saved'), type: 'success' });
                    this.render();
                } catch (error) {
                    NyouUI.Toast({
    ...
    renderPersonalization(container) {
        const wallpapers = [
            'Theme/Picture/Fluent-1.webp', 'Theme/Picture/Fluent-2.webp', 'Theme/Picture/Fluent-3.webp',
            'Theme/Picture/Fluent-4.webp', 'Theme/Picture/Fluent-5.webp', 'Theme/Picture/Fluent-6.webp'
        ];
        ...
        // 1. Bing 壁纸按钮
        const bingBtn = NyouUI.Button({
            text: t('settings.bing-wallpaper'),
            icon: 'Image',
            onClick: async () => {
                // 添加加载状态
                const btn = bingBtn.querySelector('button') || bingBtn; // NyouUI.Button 返回的是 button 元素
        ...
        const lockBingBtn = NyouUI.Button({
            text: t('settings.bing-wallpaper'),
            icon: 'Globe',
            onClick: async () => {
                try {
        ...
        const lockUploadBtn = NyouUI.Button({
        ...
        // 主题选择 - 使用 NyouUI.SettingItem + NyouUI.Select
        appearanceSection.appendChild(NyouUI.SettingItem({
        ...
        // 自动重启
        const restartSection = this.createSection('自动重启');
        const everyVisit = State.settings.autoRestartEveryVisit !== false;
        const restartToggleItem = NyouUI.SettingItem({
            label: '每次进入都重启',
            description: '打开网页时自动重启系统。',
            control: NyouUI.Toggle({
                checked: everyVisit,
                onChange: (value) => {
                    const on = value === true;
                    State.updateSettings({ autoRestartEveryVisit: on });
                    const intervalRow = document.getElementById('settings-autorestart-interval');
                    if (intervalRow) intervalRow.style.display = on ? 'none' : 'flex';
                }
            })
        });
        restartSection.appendChild(restartToggleItem);

        const intervalRow = document.createElement('div');
        intervalRow.id = 'settings-autorestart-interval';
        intervalRow.style.cssText = `display:${everyVisit ? 'none' : 'flex'};align-items:center;gap:8px;padding:12px 16px;flex-wrap:wrap`;
        intervalRow.innerHTML = `
            <span style="font-size:13px;color:var(--text-secondary)">每隔</span>
            <input type="number" id="settings-ar-hours" min="0" max="999" value="${State.settings.autoRestartHours || 0}" style="width:56px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;text-align:center;background:var(--bg);color:var(--text)">
            <span style="font-size:13px">小时</span>
            <input type="number" id="settings-ar-minutes" min="0" max="59" value="${State.settings.autoRestartMinutes || 0}" style="width:56px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;text-align:center;background:var(--bg);color:var(--text)">
            <span style="font-size:13px">分钟</span>
            <input type="number" id="settings-ar-seconds" min="0" max="59" value="${State.settings.autoRestartSeconds ?? 30}" style="width:56px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;text-align:center;background:var(--bg);color:var(--text)">
            <span style="font-size:13px;color:var(--text-secondary)">秒再次进入此网页将重启。</span>
        `;
        restartSection.appendChild(intervalRow);
        container.appendChild(restartSection);

        // 绑定时间输入事件
        setTimeout(() => {
            const h = document.getElementById('settings-ar-hours');
            const m = document.getElementById('settings-ar-minutes');
            const s = document.getElementById('settings-ar-seconds');
            if (h) h.addEventListener('change', () => State.updateSettings({ autoRestartHours: Number(h.value) || 0 }));
            if (m) m.addEventListener('change', () => State.updateSettings({ autoRestartMinutes: Number(m.value) || 0 }));
            if (s) s.addEventListener('change', () => State.updateSettings({ autoRestartSeconds: Number(s.value) || 0 }));
        }, 0);

        const advancedSection = this.createSection(t('settings.advanced-options'));
        const advancedItem = NyouUI.SettingItem({
    ...
    renderPersonalizationAdvanced(container) {
        container.className = 'settings-content settings-material-page settings-secondary-page';

        container.appendChild(this.createSecondaryBackBar(t('settings.personalization'), () => {
    ...
    renderLab(container) {
        ...
        section.appendChild(NyouUI.SettingItem({
            ...
            control: NyouUI.Toggle({
                ...
                onChange: (v) => {
                    ...
                    this.addRecentSetting(t('settings.tombstone-dim-frozen-apps'), v ? t('settings.on') : t('settings.off'), 'lab');
                    State.addNotification({
                        title: t('settings.lab-title'),
        ...
        const experimentalToggles = [
            {
                key: 'enableSystemInteractionAudio',
                label: 'settings.lab-system-interaction-audio',
                description: 'settings.lab-system-interaction-audio-desc',
                enabledMessage: 'settings.lab-system-interaction-audio-on',
                disabledMessage: 'settings.lab-system-interaction-audio-off'
            },
            {
                key: 'enableCrossAppDragDrop',
                label: 'settings.lab-cross-app-drop',
                description: 'settings.lab-cross-app-drop-desc',
                enabledMessage: 'settings.lab-cross-app-drop-on',
                disabledMessage: 'settings.lab-cross-app-drop-off'
            },
            {
                key: 'enableAppMultiWindow',
                label: 'settings.lab-multi-window',
                description: 'settings.lab-multi-window-desc',
                enabledMessage: 'settings.lab-multi-window-on',
                disabledMessage: 'settings.lab-multi-window-off'
            }
        ];

        experimentalToggles.forEach((item) => {
            section.appendChild(NyouUI.SettingItem({
                label: t(item.label),
                description: t(item.description),
                control: NyouUI.Toggle({
                    checked: State.settings[item.key] === true,
                    onChange: (value) => {
                        State.updateSettings({ [item.key]: value });
                        this.addRecentSetting(t(item.label), value ? t('settings.on') : t('settings.off'), 'lab');
                        State.addNotification({
                            title: t('settings.lab-title'),
                            message: t(value ? item.enabledMessage : item.disabledMessage),
                            type: 'info',
                            playSound: item.key !== 'enableSystemInteractionAudio'
                        });
                    }
                })
            }));
        ...
        lingyiNotice.innerHTML = `
            <img src="Theme/Icon/Symbol_icon/stroke/Exclamation Triangle.svg" alt="" style="width: 20px; height: 20px; opacity: 0.8;">
            <span style="display:grid; gap:3px; font-size:13px; color:var(--text-secondary);"><strong style="color:#c42b1c;">${t('settings.lingyi-deprecated')}</strong><span>${t('settings.lingyi-notice')}</span></span>
        `;
        lingyiNotice.style.cssText = 'display: flex; align-items: center; gap: 10px; padding: 12px 16px; background: color-mix(in srgb, #c42b1c 9%, var(--bg-secondary)); border: 1px solid color-mix(in srgb, #c42b1c 24%, var(--border-color)); border-radius: var(--radius-md); margin-bottom: 16px;';
        lingyiSection.appendChild(lingyiNotice);
        
        // 灵翼交互主开关
        const lingyiEnabled = State.settings.lingyiEnabled || false;
        lingyiSection.appendChild(NyouUI.SettingItem({
    ...
    renderApplications(container) {
        ...
        const preparedSnapshot = this._lastStorageSnapshot;
        this.renderAppsListContent(appsList, apps, preparedSnapshot);
        section.appendChild(appsList);
        
        container.appendChild(section);

        if (preparedSnapshot) {
            this.renderStorageOverview(storageCard, preparedSnapshot);
        } else if (globalThis.NyouOSStorage) {
            this.getPreparedStorageSnapshot().then((snapshot) => {
    ...
    getStorageApps() {
        ...
        return desktopApps.filter((app) => app.isPWA !== true || app.developerCreated === true || installed.has(app.id)).map((app) => {
    ...
    renderStorageOverview(card, snapshot) {
        ...
        const displayQuota = Number(snapshot.displayCapacityBytes)
            || NyouOSStorage.getDisplayCapacity(snapshot.importedFilesBytes || 0);
        ...
        usage.textContent = `${NyouOSStorage.formatBytes(snapshot.browserUsageBytes || 0)} / ${NyouOSStorage.formatBytes(displayQuota)}`;
    },

    renderAppsListContent(container, apps, snapshot) {
        const allApps = apps.map((app) => {
            const sizeBytes = snapshot?.apps?.[app.id]?.appSizeBytes
                ?? (globalThis.NyouOSStorage ? NyouOSStorage.getAppSize(app) : 0);
            const dataSizeBytes = snapshot?.apps?.[app.id]?.dataSizeBytes ?? 0;
            return {
                ...app,
                sizeBytes,
                dataSizeBytes,
                dataAvailable: snapshot?.apps?.[app.id]?.dataAvailable !== false,
        ...
        container.innerHTML = allApps.map(app => `
            <div class="app-list-item" data-app-id="${app.id}" data-is-pwa="${app.isPWA}">
                ...
                <div class="app-size">${app.dataAvailable ? (globalThis.NyouOSStorage ? NyouOSStorage.formatBytes(app.totalSizeBytes) : '0 B') : t('settings.storage-unknown')}</div>
                <div class="app-arrow">
    ...
    showAppDetail(app) {
        this.currentAppDetail = app;
        // 保持同一个 NyouWindow 实例，让框架保存 applications 页的滚动位置。
        this.navigateToPage('app-detail');
    },
    ...
    renderAppDetailPage(container) {
        ...
        container.className = 'settings-content settings-secondary-page';

        const section = document.createElement('div');
        section.className = 'settings-section app-detail-page';
        
        container.appendChild(this.createSecondaryBackBar(t('settings.applications'), () => {
            // 播放滑出动画
        ...
        storageCard.innerHTML = `
            <div class="app-storage-info">
                <div class="app-storage-row">
                    ...
                    <span data-storage-value="app">${NyouOSStorage.formatBytes(initialSize)}</span>
                </div>
                <div class="app-storage-divider"></div>
                <div class="app-storage-row">
                    <span>${t(app.id === 'files' ? 'settings.documents-data' : 'settings.app-data')}</span>
                    <span data-storage-value="data">${NyouOSStorage.formatBytes(initialDataSize)}</span>
                </div>
                <div class="app-storage-divider"></div>
                <div class="app-storage-row app-storage-total">
                    <span>${t('settings.app-total')}</span>
                    <span data-storage-value="total">${NyouOSStorage.formatBytes(initialSize + initialDataSize)}</span>
                </div>
            </div>
        `;
        section.appendChild(storageCard);
        ...
        repairHost.appendChild(NyouUI.Button({
        ...
        uninstallHost.appendChild(NyouUI.Button({
        ...
        if (globalThis.NyouOSStorage) {
            const applySnapshot = (snapshot) => {
                ...
                storageCard.querySelector('[data-storage-value="app"]').textContent = NyouOSStorage.formatBytes(values.appSizeBytes);
                storageCard.querySelector('[data-storage-value="data"]').textContent = values.dataAvailable === false
                    ? t('settings.storage-unknown')
                    : NyouOSStorage.formatBytes(values.dataSizeBytes);
                storageCard.querySelector('[data-storage-value="total"]').textContent = values.dataAvailable === false
                    ? t('settings.storage-unknown')
                    : NyouOSStorage.formatBytes(values.appSizeBytes + values.dataSizeBytes);
            };
            if (this._lastStorageSnapshot) applySnapshot(this._lastStorageSnapshot);
            else this.getPreparedStorageSnapshot().then(applySnapshot);
        }
    },
    ...
    repairApp(app) {
        ...
        if (this.repairingApps.includes(app.id)) {
            NyouUI.Toast({ title: app.name, message: t('settings.app-repairing'), type: 'warning' });
            return;
        }
    ...
    renderAbout(container) {
        ...
        heroCard.innerHTML = `
            <div class="settings-about-landscape" aria-hidden="true">
                <img class="settings-about-landscape-layer settings-about-landscape-sky" src="Theme/Qingshan_lake/03_sky_far_background_mountains.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-water-base" src="Theme/Qingshan_lake/04_water_base_gradient_bands.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-reflection" src="Theme/Qingshan_lake/05_water_reflection_bands.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-mountain-middle" src="Theme/Qingshan_lake/06_middle_colored_mountain_slopes.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-mountain-dark" src="Theme/Qingshan_lake/07_dark_mountain_peaks.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-land" src="Theme/Qingshan_lake/09_foreground_dark_land.webp" alt="">
                <img class="settings-about-landscape-layer settings-about-landscape-trees" src="Theme/Qingshan_lake/10_tree_silhouettes.webp" alt="">
            </div>
            <div class="settings-about-brand">
                <div class="settings-about-brand-logo">
                    <img class="settings-about-logo-light" src="Theme/Icon/Nyou_logo.webp" alt="">
                    <img class="settings-about-logo-dark" src="Theme/Icon/Nyou_logo_dark.webp" alt="">
                </div>
        ...
        const projectUrl = 'https://nyouos.pages.dev';
        ...
        // 系统信息卡片
        const sysInfoCard = document.createElement('div');
        sysInfoCard.className = 'fluent-setting-item settings-about-meta-card';
        const isDark = document.body && document.body.classList.contains('dark-mode');
        const langName = (I18n.languages || []).find(l => l.code === I18n.currentLang)?.nativeName || I18n.currentLang;
        ...
        sysInfoCard.innerHTML = `
            <div class="settings-about-meta-row" style="font-weight:600;margin-bottom:10px;font-size:14px;">${t('settings.about.system-info')}</div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">${t('settings.about.uptime')}</div>
                <div class="settings-about-meta-value" id="sys-uptime">00:00:00</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">${t('settings.about.screen-resolution')}</div>
                <div class="settings-about-meta-value">${screen.width} × ${screen.height}</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">${t('settings.about.browser')}</div>
                <div class="settings-about-meta-value">${browserName}</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">${t('settings.about.current-language')}</div>
                <div class="settings-about-meta-value">${langName}</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">${t('settings.about.theme-mode')}</div>
                <div class="settings-about-meta-value">${isDark ? t('settings.theme-dark') : t('settings.theme-light')}</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">后端状态</div>
                <div class="settings-about-meta-value" id="backend-status">检测中...</div>
            </div>
            <div class="settings-about-meta-row">
                <div class="settings-about-meta-label">后端区域</div>
                <div class="settings-about-meta-value" id="backend-region">-</div>
            </div>
        `;
        section.appendChild(sysInfoCard);
        ...
        fetch('/api/status')
            ...
            .then(data => {
                ...
                const regionEl = document.getElementById('backend-region');
                if (statusEl) statusEl.textContent = data.status === 'ok' ? '✅ 在线 (Cloudflare Pages Functions)' : '❌ 离线';
                if (regionEl) regionEl.textContent = data.backend?.region || '-';
            })
            .catch(() => {
                const statusEl = document.getElementById('backend-status');
                if (statusEl) statusEl.textContent = '❌ 离线 (本地预览)';
            });

        // 实时更新运行时间
        const _sysStartTime = Date.now();
        const _updateSysUptime = () => {
            const elapsed = Math.floor((Date.now() - _sysStartTime) / 1000);
        ...
        _updateSysUptime();
        setInterval(_updateSysUptime, 1000);

        const list = NyouUI.List({
            ...
            items: [
                ...
                { id: 'license', title: t('settings.license'), description: '闭源软件 © 2025-2026 KevinAnanda', icon: 'Document' }
            ],
            onItemClick: (id) => {
    ...
    renderAboutChangelog(container) {
        container.className = 'settings-content settings-about-changelog settings-secondary-page';

        const section = document.createElement('section');
        section.className = 'settings-section settings-changelog-page';

        container.appendChild(this.createSecondaryBackBar(t('settings.about-title'), () => {
            section.classList.add('slide-out');
    ...
    createAccentColorPanel() {
        ...
        const autoToggle = NyouUI.Toggle({
            checked: autoEnabled,
            onChange: async (next) => {
        ...
        const readabilityToggle = NyouUI.Toggle({
